...
if [[ "${VALIDATION_ACK:-}" != "$ACK_REQUIRED" ]]; then
  echo "BLOCKED_WATCH_QUALITY_VALIDATION: VALIDATION_ACK must equal $ACK_REQUIRED" >&2
  exit 2
fi
if [[ -z "$SOURCE_REF" || ! "$SOURCE_REF" =~ ^[0-9a-f]{40}$ ]]; then
  echo "BLOCKED_WATCH_QUALITY_VALIDATION: WATCH_QUALITY_SOURCE_REF must be an exact 40-character commit SHA" >&2
  exit 2
fi
if [[ ! -d "$HOST_REPO/.git" ]]; then
  echo "BLOCKED_WATCH_QUALITY_VALIDATION: repository unavailable: $HOST_REPO" >&2
  exit 2
fi
for required in "$TAR" "$NPM" "$NPX" "$PYTHON_DEFAULT"; do
  if [[ -z "$required" || ! -x "$required" ]]; then
    echo "BLOCKED_WATCH_QUALITY_VALIDATION: required existing command unavailable" >&2
    exit 2
  fi
done
...
if [[ "$RESOLVED_COMMIT" != "$SOURCE_REF" ]]; then
  echo "BLOCKED_WATCH_QUALITY_VALIDATION: resolved commit differs from exact source ref" >&2
  exit 2
fi
...
markers=(
  ...
  "OWNERSHIP ELIGIBLE"
  "MECHANICS VALID"
  "DETERMINISTIC REVIEW REQUIRED"
  "RECOMMENDED · NOT RUN"
)
for marker in "${markers[@]}"; do
  if ! grep -R --binary-files=text -Fq "$marker" "$STAGE_ROOT/apps/command-center-v3/dist"; then
    echo "BLOCKED_WATCH_QUALITY_VALIDATION: built bundle missing marker: $marker" >&2
    exit 4
  fi
done
