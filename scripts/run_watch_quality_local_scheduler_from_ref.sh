...
if [[ -z "$GATE4_JSON" ]]; then
  GATE4_JSON="$(find "$EVIDENCE_ROOT" -maxdepth 1 -type f -name 'watch-quality-gate4-*.json' -printf '%T@ %p\n' 2>/dev/null | sort -nr | awk 'NR==1 {sub(/^[^ ]+ /, ""); print; exit}')"
fi
[[ -n "$GATE4_JSON" && -f "$GATE4_JSON" ]] && grep -Fq '"status": "PASS_GATE4_READONLY_VERIFICATION"' "$GATE4_JSON" || { echo "BLOCKED_GATE6: passing Gate 4 evidence not found" >&2; exit 2; }

BUILD_META="$HOST_REPO/apps/command-center-v3/dist/build-meta.json"
[[ -f "$BUILD_META" ]] || { echo "BLOCKED_GATE6: live build-meta.json unavailable" >&2; exit 2; }
