"""Persistent shadow motion engine for Active Trader.

A tick consumes already-available read models plus local journal observations, restores
T2 lease state, evaluates momentum-exit hysteresis for open shadow/paper positions, and
writes one aggregate snapshot. It has no market-data client and no broker/order path.
"""
...
def default_snapshot_path() -> Path:
    ...
    return Path(env).expanduser() if env else _repo_root() / "data" / "active_trader" / "motion_snapshot.json"


def default_state_path() -> Path:
    env = os.environ.get("ACTIVE_TRADER_MOTION_STATE", "").strip()
    return Path(env).expanduser() if env else _repo_root() / "data" / "active_trader" / "motion_state.json"
...
def _explicit_symbols(session: Mapping[str, Any]) -> set[str]:
    ...
    for container in (
        session,
        session.get("envelope") if isinstance(session.get("envelope"), Mapping) else {},
        session.get("draft") if isinstance(session.get("draft"), Mapping) else {},
    ):
        ...
        if isinstance(raw_rule, (list, tuple, set, frozenset)):
            ...
            if text.lower().startswith("symbols:"):
                text = text.split(":", 1)[1]
            # Fail closed on wildcard/rule expressions; accept only explicit CSV tokens.
...
def _session_context(session: Mapping[str, Any] | None) -> dict[str, Any]:
    ...
    return {
        ...
        "state": state,
        "mode": mode,
        "active": state == "ACTIVE" and mode in ALLOWED_MODES,
...
def _candidate_observation(
    ...
    *,
    ...
    baseline_age = (
        ...
        else (max(0.0, now - observed_at) if observed_at is not None else float("inf"))
    )
    ...
    gate = str(raw.get("gate_decision") or raw.get("gateDecision") or raw.get("gate") or "").upper()
    if gate not in {"PASS", "VETO"}:
        gate = "DEFER"

    allowed = bool(ctx["active"] and symbol and symbol in ctx["symbols"])
    ...
    return CandidateObservation(
        ...
        trigger_distance_bps=_optional_float(
            raw.get("trigger_distance_bps") or raw.get("triggerDistanceBps")
        ),
        expected_fire_in_s=_optional_float(
            raw.get("expected_fire_in_s") or raw.get("expectedFireInS")
        ),
        operator_selected=bool(raw.get("operator_selected") or raw.get("operatorSelected")),
...
def _position_key(payload: Mapping[str, Any]) -> str:
    return str(payload.get("position_id") or payload.get("positionId") or payload.get("symbol") or "").strip()
...
class MotionEngine:
    def __init__(
        ...
        *,
        ...
        self.snapshot_path = Path(snapshot_path).expanduser() if snapshot_path else default_snapshot_path()
    ...
    def _load_state(self) -> dict[str, Any]:
        try:
            ...
            return data if isinstance(data, dict) and data.get("contract") == STATE_CONTRACT else {}
        except Exception:
            return {}
    ...
    def _manager(self) -> T2LeaseManager:
        ...
        for raw in leases:
            if not isinstance(raw, Mapping):
                continue
            try:
                lease = T2Lease(**{k: raw[k] for k in (
                    "lease_id", "symbol", "admitted_at", "renewed_at", "expires_at",
                    "priority", "position_open"
                )})
        ...
        manager._cooldown_until = {
            str(k): float(v)
            for k, v in (cooldowns.items() if isinstance(cooldowns, Mapping) else [])
        ...
        *,
        ...
        for position_id, history in sorted(self._position_histories().items()):
            ...
            for payload in history:
                try:
                    decision = policy.evaluate(MomentumObservation.from_mapping(payload))
                except Exception:
                    continue
            if decision is None:
                continue
    ...
    def tick(
        ...
        *,
        ...
        if not verification.ok:
            raise JournalIntegrityError(verification.error or "motion journal invalid")
        ...
        # Explicit journal observations override weaker scanner/read-model projections.
        merged.update(self._latest_candidate_payloads())

        positions, exit_signals, position_candidates = self._evaluate_positions(now=now)
        ...
        observations.extend(
            _candidate_observation(raw, session, now=now, position_open=True)
        ...
        for symbol, raw in sorted(merged.items()):
            ...
            candidate_rows.append({
                ...
                "reason_code": decision.reason_code if decision else "not_evaluated",
                "refresh_after_s": decision.refresh_after_s if decision else self.t2_config.idle_refresh_s,
                "priority": decision.priority if decision else 0.0,
                "last_update_age_s": max(0.0, now - observed_at) if observed_at is not None else None,
                "observed_at": observed_at,
                "price": _optional_float(raw.get("price") if raw.get("price") is not None else raw.get("last")),
                "setup_state": raw.get("setup_state") or raw.get("setupState") or raw.get("state"),
                "gate_decision": raw.get("gate_decision") or raw.get("gateDecision") or raw.get("gate"),
        ...
        snapshot = {
            "contract": CONTRACT,
            "generated_at": now,
            "data_state": "LIVE_MOTION" if candidate_rows or positions else "EMPTY_LIVE_MOTION",
            "available": True,
            "ui_refresh_after_s": int(refresh),
            "push_primary": bool(t2_snapshot.push_primary),
            "max_pull_fallbacks_per_minute": int(t2_snapshot.max_pull_fallbacks_per_minute),
            "session": {
                "session_id": session_ctx["session_id"],
                "state": session_ctx["state"],
                "mode": session_ctx["mode"],
                "execution_ready": bool(session_ctx["active"]),
                "authorized_symbol_count": len(session_ctx["symbols"]),
            },
            "t2": {
                ...
                "decisions": [decision.to_dict() for decision in t2_snapshot.decisions],
            ...
            "authority": dict(_ZERO_AUTHORITY),
            "note": "Shadow/read snapshot only. Exit signals are evidence; no automatic order is sent.",
        }
