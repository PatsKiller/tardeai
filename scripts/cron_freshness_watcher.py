...
CHECKS = [
    # (label, type, sql/filesystem_check, max_age_hours, retry_cmd)
    {
        ...
        "max_hours": 12.0,
        "retry_cmd": "flock -n /tmp/finnhub_ingest.lock .venv/bin/python scripts/news_ingestion.py >> logs/finnhub_ingest.log 2>&1",
        "retry_timeout_s": 300,
    },
    {
        "label": "yahoo_finance",
    ...
    },
    {
        ...
        "max_hours": 6.0,
        "retry_cmd": "flock -n /tmp/finviz_quotes.lock .venv/bin/python scripts/portfolio_repricer.py >> logs/finviz_quotes.log 2>&1",
        "retry_timeout_s": 300,
    },
    {
    ...
    },
    {
        ...
        "check": "SELECT EXTRACT(EPOCH FROM (NOW() - MAX(updated_at)))/3600.0 FROM watchlist_items WHERE updated_at IS NOT NULL",
        "max_hours": 8.0,
        "retry_cmd": "flock -n /tmp/entry_planner.lock .venv/bin/python scripts/watchlist_entry_planner.py --main-lane --limit 40 --lane local --no-alert >> logs/entry_planner.log 2>&1",
        "retry_timeout_s": 600,
    },
    {
        "label": "schwab_sync",
        ...
        "max_hours": 24.0,
        "retry_cmd": "flock -n /tmp/schwab_sync.lock .venv/bin/python scripts/schwab_position_sync.py >> logs/schwab_sync.log 2>&1",
        "retry_timeout_s": 300,
    },
    {
