...
def eligible_universe(
    ...
    *,
    ...
    """Priority order: held → starred → defense buy → top ideas → street SB/B → event.

    Defense get_into/pairs use the same priority band as Street BUY/Strong Buy.
    ...
    held, starred, defense_buy, top, street, events = [], [], [], [], [], []
    seen: set[str] = set()
    ...
    def accept(c: dict) -> bool:
        ...
        if is_unresolved_identity(c):
            return False
        # Quarantine / CECO containment — never schedule
        try:
            from lib.data_broker.watch_domains import load_review_artifacts, QUARANTINE

            arts = load_review_artifacts((c.get("symbol") or "").upper())
            for a in arts.values():
                if (a or {}).get("artifact_disposition") == "QUARANTINED":
                    return False
                if (a or {}).get("reason_code") == "UNVERIFIED_OPERATOR_AUTHORIZATION":
                    return False
            sym_u = (c.get("symbol") or "").upper()
            if sym_u and QUARANTINE.exists() and any(QUARANTINE.glob(f"{sym_u}_*")):
                return False
        except Exception:
            pass
        return True
    ...
    # Defense desk buy-side (get_into/pairs) — same band as Street BUY/Strong Buy
    for c in cards:
        if not accept(c):
            continue
        sym = (c.get("symbol") or "").upper()
        if not sym or sym in seen:
            continue
        if c.get("defense_buy"):
            defense_buy.append({"symbol": sym, "card": c})
            seen.add(sym)
    ...
    for c in cards:
        ...
        if not sym or sym in seen:
            continue
        # Street BUY/Strong Buy OR defense_buy already handled above
        if (c.get("street_rating") or "") in ("STRONG BUY", "BUY") or c.get("defense_buy"):
    ...
    ordered = held + starred + defense_buy + top + street + events
...
def review_freshness_fields(card: dict[str, Any]) -> dict[str, Any]:
    ...
    if is_unresolved_identity(card):
        ...
        maria.get("artifact_disposition") == "QUARANTINED"
    ...
    return {
        ...
        if blocked_reason
            ...
            "COMPLETE" if maria.get("status") == "COMPLETE" else ("POLICY_CONFIGURED" if not enabled else "SCHEDULED")
        ),
        "execution_status": "DISABLED" if not enabled else ("BLOCKED" if blocked_reason else "ENABLED"),
        ...
        "review_sla_state": "BLOCKED"
        if blocked_reason
        else (maria_sla if maria_sla == "OVERDUE" else ("OVERDUE" if cio_sla == "OVERDUE" else maria_sla)),
        ...
        "last_attempt_status": (
            "COMPLETE"
            if maria.get("status") == "COMPLETE" or cio.get("status") == "COMPLETE"
            else (
                blocked_reason or ("WORKERS_DISABLED" if not enabled else (maria.get("reason_code") or "NOT_SCHEDULED"))
            )
        ),
    }
...
def enrich_review_display(rev: dict[str, Any], *, agent: str, card: dict) -> dict[str, Any]:
    ...
    r["execution_status"] = "DISABLED" if not enabled else ("BLOCKED" if review_status == "BLOCKED" else "QUEUED")
    r["next_review_at"] = policy_window  # policy window (not a promise to execute)
    r["next_execution_at"] = next_exec
    r["next_policy_window"] = policy_window
    r["reason_code"] = (
        detail_reason if not (r.get("reason_code") in ("UNVERIFIED_OPERATOR_AUTHORIZATION",)) else r.get("reason_code")
    )
    ...
    *,
    ...
    for item in universe:
        ...
        if maria_count >= max_maria:
            deferred.append(
                {"symbol": sym, "agent_id": "maria", "reason": "COST_DEFERRED", "next": times["next_maria_review_at"]}
            )
        else:
        ...
        m = arts.get("maria") or {}
        # load_review_artifacts normalizes disposition COMPLETE for authorized complete rows
        if m.get("status") == "COMPLETE" and (m.get("artifact_disposition") or "COMPLETE") == "COMPLETE":
            if m.get("artifact_disposition") != "QUARANTINED":
                maria_ok = True
        ...
        if cio_count >= max_cio:
            deferred.append(
                {"symbol": sym, "agent_id": "cio", "reason": "COST_DEFERRED", "next": times["next_cio_review_at"]}
            )
            continue
