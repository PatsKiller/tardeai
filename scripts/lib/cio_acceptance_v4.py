...
ACCEPTANCE_VERSION = "cio_acceptance_v4.0.0"
...
def finalize_verdict(
    ...
    *,
    ...
    result = {
        ...
        "pass_threshold": production_pass,  # kept for CLI compat; equals PRODUCTION_ACCEPTANCE
...
def eval_g1_exact_live_sha(*, live_sha: str, main_sha: str, now: Optional[datetime] = None) -> dict:
    ok = bool(live_sha) and bool(main_sha) and _full_sha(live_sha) == _full_sha(main_sha)
    return make_gate(
        "G1_exact_live_sha",
        expected="origin/main SHA == live BUILD_SHA (full)",
        actual=f"main={_sha12(main_sha)} live={_sha12(live_sha)}",
        status="PASS" if ok else "FAIL",
        reason=(
            "Live backend SHA equals origin/main."
            if ok
            else "Live backend is not exact current main. Ancestor/RC lag is FAIL."
        ),
        severity="P0",
...
def eval_g3_drive_manifest_parity(
    *,
    ...
    """Drive parity. Unproven or duplicate canonical names → FAIL (not PASS)."""
    if not drive_proven:
        return make_gate(
            "G3_drive_manifest_parity",
...
def eval_g4_financial_book(
    *,
    ...
    ok = verified and cash_ok and accts_ok and ok_flag is True
    return make_gate(
        "G4_financial_book_reconciliation",
        expected="overall_quality in {VERIFIED_CURRENT, VERIFIED_AS_OF} and book invariants true",
        actual={
...
def eval_g5_zero_material_conflicts(
    *,
    ...
    opaque_conflicts = (count or 0) > 0 and not material and not symbols
    ok = len(material) == 0 and len(symbols) == 0 and (count or 0) == 0 and not dirty_quality
    return make_gate(
        "G5_zero_material_price_conflicts",
        expected="zero material price/MV/UPL conflicts on the production book",
        actual={
    ...
    *,
    ...
    checks = {
        ...
        "ledger_invariants_ok": (
            inv.get("earmark_le_settled_cash", True) is True
            and inv.get("deploy_le_free_plus_prospective", True) is True
        ),
    }
...
def eval_g10_g12_report_formats(
    *,
    ...
    def _fmt(gid: str, path: str, label: str, sev: str = "P0") -> dict:
        exists = bool(path)
        ok = exists and (not synthetic) and sha_ok
        return make_gate(
            gid,
            expected=f"live-portfolio {label} generated; source_sha == live SHA",
...
def eval_g13_visual_qa(
    *,
    ...
    ok = bool(visual_qa_artifact) and pages_inspected > 0
    return make_gate(
        "G13_report_visual_qa",
        expected="every live PDF page rendered and visually inspected",
        actual={"artifact": visual_qa_artifact or None, "pages_inspected": pages_inspected},
        status="PASS" if ok else "FAIL",
        reason=(
            f"Visual QA recorded for {pages_inspected} pages."
            if ok
            else "No visual QA evidence for the live PDF (unproven ≠ pass)."
        ),
        severity="P1",
...
def eval_g16_zero_duplicate(
    *,
    ...
    ev = canary_evidence or {}
    ok = ev.get("repeat_unchanged_sends") == 0 and ev.get("sent") is True
    return make_gate(
        "G16_zero_duplicate_notification",
        expected="repeat unchanged canary sends == 0",
        actual={"repeat_unchanged_sends": ev.get("repeat_unchanged_sends"), "sent": ev.get("sent")},
        status="PASS" if ok else "FAIL",
        reason=(
...
def eval_g17_authority(
    *,
    ...
    return make_gate(
        "G17_authority_read_only",
        expected="READ_ONLY_ADVISORY on every CIO surface",
        actual={"bad": bad, "checked": [s.get("name") for s in (surfaces or [])]},
        status="PASS" if ok else "FAIL",
...
def evaluate_live_snapshot(snap: dict[str, Any], *, now: Optional[datetime] = None) -> dict[str, Any]:
    ...
    gates: list[dict[str, Any]] = []
    gates.append(eval_g1_exact_live_sha(live_sha=live, main_sha=main, now=now))
    ...
    gates.append(eval_g3_drive_manifest_parity(
        ...
        drive_duplicate_count=int(snap.get("drive_duplicate_count") or 0),
        drive_proven=bool(snap.get("drive_proven")),
        now=now,
    ))
    ft = snap.get("financial_truth_gate") or {}
