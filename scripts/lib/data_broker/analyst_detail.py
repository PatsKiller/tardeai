...
def get_analyst_targets(db_query, symbols: list[str]) -> dict[str, dict[str, Any]]:
    """Return {SYMBOL: {target_mean, target_low, target_high, recommendation_key, number_of_analysts}}
    ...
    rows = db_query(
        """SELECT DISTINCT ON (upper(symbol))
                  ...
                  recommendation_key, number_of_analysts, fetched_at
    ...
    for row in rows:
        ...
        if sym:
            out[sym] = {
                ...
                "analyst_count": row.get("number_of_analysts"),
