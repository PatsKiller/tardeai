...
# Desk Path B strategies are generated by options_engine for Schwab Path B + 2FA.
# Paper-lab strategies are educational / Alpaca research lanes. The Hub Strategy
# Validation strip must not paint Path B strategies as "PAPER MODEL" just because
# their paper-outcome ledger is empty (operator closeout 2026-09-25).
DESK_PATH_B_STRATEGIES = frozenset({
    "covered_call", "cash_secured_put", "protective_put", "credit_spread",
    "long_call", "long_put",
})
...
def strategy_lane(strategy_id: str) -> str:
    """paper_lab | desk_path_b | other — for Hub chrome only; never enables live."""
    sid = str(strategy_id or "").strip()
    if sid in PAPER_LAB_STRATEGIES:
        return "paper_lab"
    if sid in DESK_PATH_B_STRATEGIES:
        return "desk_path_b"
    # Registry additions default to paper_lab chrome until explicitly promoted.
    return "paper_lab"
...
def validation_status(strategy_id: str = STRATEGY_ID,
    ...
    lane = strategy_lane(strategy_id)
    n_need = int(verdict["gate"]["min_closed_paper_trades"])
    n_closed = int(metrics.get("n_closed") or 0)
    if lane == "desk_path_b":
        # Desk Path B readiness is enterprise.live_eligible + Path B 2FA — not paper n/30.
        progress_label = (
            f"desk Path B · paper ledger {n_closed}/{n_need} (lab only; does not unlock live)"
        )
        # Keep gate_met math for the lab ledger, but message must not imply paper unlocks Path B.
        if verdict.get("gate_met"):
            message = (
                "paper lab ledger met — Schwab Path B still requires enterprise live_eligible "
                "+ per-order 2FA (human_approval_required; nothing auto-enabled)"
            )
        else:
            message = (
                f"desk Path B strategy — live eligibility is liquidity/enterprise gates, "
                f"not paper outcomes ({n_closed}/{n_need} lab ledger only)"
            )
        verdict = {**verdict, "message": message}
    else:
        progress_label = f"paper validation {n_closed}/{n_need}"
    return {
        "ok": True,
        "strategy_id": strategy_id,
        "display_name": cfg["display_name"],
        "lifecycle_status": cfg["status"],
        "paper_only": cfg["paper_only"] if lane != "desk_path_b" else False,
        "lane": lane,
