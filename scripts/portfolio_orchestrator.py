...
def run_portfolio_pipeline(project_root, run_label="manual", generate_report=True, run_type="daily"):
    ...
    try:
        ...
        if _obs_today:
            ...
            _summary, _orch_lane = generate_cloud(
                _prompt, process_id="portfolio_orchestrator",
                task_summary="daily factual surveillance summary", timeout=30,
                max_tokens=200,
            )
            # Strip think tags
            ...
            if _is_valid:
                ...
                save_advisor_observations([{
                    ...
                    "evidence": {"observation_count": len(_obs_today), "escalation_count": len(_esc_today),
                                 "model": _orch_lane, "prompt_tokens": len(_prompt.split())},
                    "source": f"cloud:{_orch_lane}:enrichment",
                    "confidence": 0.85,
                    "model": f"cloud:{_orch_lane}",
