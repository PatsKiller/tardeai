...
def to_markdown(state: dict) -> str:
    ...
    lines = [
        ...
        f"- **Latest commit:** `{commit or 'unknown'}`",
        f"- **Execution state generated:** {state.get('generated_at')}",
        "- **Release manifest:** `docs/project/RELEASE_MANIFEST_LATEST.md` (see its own generated time)",
        "- **Maturity score:** `docs/diligence/current/MATURITY_SCORE_LATEST.md` / "
        "`data/runtime/maturity_score_latest.json`",
        "- **Diligence pack:** `docs/diligence/current/` (regenerated by "
        "`scripts/export_diligence_evidence.py`)",
        "",
