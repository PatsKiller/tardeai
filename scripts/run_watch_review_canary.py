...
_MAIN_ART = Path(
    "/home/johnclaw/trade-ai-v12-rebuild/trade-ai-v12-rebuild/data/runtime/watchlist_intelligence/artifacts"
)
ARTIFACT_DIR = (
    _MAIN_ART if _MAIN_ART.parent.exists() else (ROOT / "data" / "runtime" / "watchlist_intelligence" / "artifacts")
)
...
def ensure_cio_caps() -> None:
    ...
    lc.ensure_schema()
    cur = lc._conn().cursor()
    # Caps only — do NOT force mode=manual (scheduled workers need automated when enabled)
    cur.execute(
...
def write_complete_artifact(
    *,
    ...
    body = {
        ...
        if isinstance(parsed.get("evidence_references"), list)
        else [],
        "raw_response": text,
    ...
    body["artifact_hash"] = _sha(
        json.dumps({k: body[k] for k in body if k != "artifact_hash"}, sort_keys=True, default=str)
    )
...
def main() -> int:
    ...
    report["trade_ai_state"] = (ctx.get("trade_ai") or {}).get("primary_state") or (ctx.get("card") or {}).get(
        "trade_ai_state"
    )
    ...
    if do_maria:
        ...
        if maria_body.get("thinking") in (None, True):
            ...
            Path(maria_body["_path"]).write_text(
                json.dumps({k: v for k, v in maria_body.items() if k != "_path"}, indent=2, default=str) + "\n"
            )
        assert_complete(maria_body)
        if maria_body.get("model") != "deepseek-v4-flash" and res["prov"].get("returned_model") not in (
            None,
            "deepseek-v4-flash",
        ):
            # prefer returned
        ...
        print(
            json.dumps(
                {
                    "phase": "maria_done",
                    "artifact_id": maria_body["artifact_id"],
                    "cost": maria_body.get("settled_cost_usd"),
                },
                indent=2,
            ),
            flush=True,
    ...
    if do_cio:
        ...
        if not maria_body:
            ...
            return 4
        print(
            json.dumps({"phase": "cio_start", "symbol": symbol, "maria": maria_body.get("artifact_id")}, indent=2),
            flush=True,
        )
        ...
        Path(cio_body["_path"]).write_text(
            json.dumps({k: v for k, v in cio_body.items() if k != "_path"}, indent=2, default=str) + "\n"
        )
        ...
        print(
            json.dumps(
                {"phase": "cio_done", "artifact_id": cio_body["artifact_id"], "cost": cio_body.get("settled_cost_usd")},
                indent=2,
            ),
            flush=True,
        )
