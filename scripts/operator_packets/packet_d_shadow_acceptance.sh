...
#   packet_d_shadow_acceptance.sh <RC_SHA>              # plan, exit 3
#   packet_d_shadow_acceptance.sh <RC_SHA> --self-check # invariants only, no DB
#   SHADOW_DSN=... packet_d_shadow_acceptance.sh <RC_SHA> --run-shadow --ack RUN-SHADOW-ACCEPTANCE-D
#
...
if [[ -z "$EXPECTED_SHA" ]]; then
  cat >&2 <<USAGE
PREPARE-ONLY: no RC SHA supplied. Nothing was run. This packet is default-disabled.
  usage: SHADOW_DSN=... $0 <RC_SHA> --run-shadow --ack $ACK_TOKEN
         $0 <RC_SHA> --self-check     # invariants only, no DB
USAGE
  exit 2
fi
