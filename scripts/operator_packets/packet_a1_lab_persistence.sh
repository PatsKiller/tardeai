...
#   * Pins to an exact RC SHA (arg 1) and refuses if repo HEAD != that SHA.
...
#   packet_a1_lab_persistence.sh <RC_SHA>                     # plan, exit 3
#   LAB_DSN=... LAB_DSN_ALLOWLIST=host:port/db \
#     packet_a1_lab_persistence.sh <RC_SHA> --execute --ack <token> [--down]
#
...
if [[ -z "$EXPECTED_SHA" ]]; then
  cat >&2 <<USAGE
PREPARE-ONLY: no RC SHA supplied. Nothing was inspected or applied.
This packet is default-disabled and refuses to mutate.
  usage: LAB_DSN=... LAB_DSN_ALLOWLIST=host:port/db \\
         $0 <RC_SHA> --execute --ack $ACK_TOKEN [--down]
USAGE
  exit 2
fi
