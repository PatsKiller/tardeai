...
def _is_production_dbname(dbname: str | None) -> bool:
    ...
    if name == "trade_ai":
        return True
    # Explicit production markers in the database name only (not path substrings).
    ...
    if name == "prod" or name.startswith("prod-") or "-prod-" in name:
        return True
    # Do NOT treat trade_ai_agentic_lab as production (contains "trade_ai" as prefix only).
    return False
...
def _shadow_dsn_guard(dsn: str) -> None:
    ...
    # Prefer parsed user; fall back to substring only for role (never for db path checks).
