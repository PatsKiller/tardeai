...
def _ollama(prompt: str, timeout: int = 180) -> str:
    """Call Ollama qwen3:1.7b for narrative generation."""
...
def _send_telegram(message: str) -> None:
    ...
    if not bot_token or not chat_id:
        print("  [weekly-report] No Telegram creds")
        return
    try:
        for cid in str(chat_id).split(","):
            ...
            with urllib.request.urlopen(req, timeout=15):
                print(f"  [weekly-report] Telegram sent to {cid}")
    except Exception as e:
...
<div class="meta">Week ending {date_str} · Generated by Ollama qwen3:1.7b · Trade AI v12</div>
...
<div class="footer">
  Generated {datetime.now().strftime('%Y-%m-%d %H:%M')} · Local LLM (Ollama qwen3:1.7b) · $0 API cost
