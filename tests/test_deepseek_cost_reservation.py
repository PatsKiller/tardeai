...
def test_settled_actual_counted_when_log_fails(monkeypatch):
    ...
    rid = lc.reserve_projected_cost(pid, 0.01, model_id="deepseek-v4-flash")
...
def test_released_pre_provider_does_not_consume(monkeypatch):
    ...
    before = lc.ledger_paid_usd_today(pid)
    rid = lc.reserve_projected_cost(pid, 0.02, model_id="deepseek-v4-flash")
...
def test_ambiguous_timeout_settles_conservatively(monkeypatch):
    ...
    before = lc.ledger_paid_usd_today(pid)
    rid = lc.reserve_projected_cost(pid, 0.03, model_id="deepseek-v4-flash")
...
def test_concurrent_reservations_respect_cap(monkeypatch):
    ...
    def worker():
        try:
            rid = lc.reserve_projected_cost(pid, 0.04, model_id="deepseek-v4-flash")
            with lock:
