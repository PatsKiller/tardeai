...
class TestModelPolicyExact(unittest.TestCase):
    ...
    def test_registry_cio_allows_pro(self):
        ...
        # A missing key means False (llm_consumption reads fallback_allowed with default False),
        # and the workers pin fallback_allowed=False per call and refuse a fallback response.
        self.assertFalse(cio.get("fallback_allowed", False))
...
class TestPipelineGuards(unittest.TestCase):
    def test_cio_requires_maria(self):
        ...
        with mock.patch(
            "lib.data_broker.watch_domains.load_review_artifacts",
            return_value={
...
class TestWorkerDisabled(unittest.TestCase):
    def test_execute_requires_allow_flag(self):
        ...
        pv = body.get("policy_validation") or ""
        # The containment gate reports "CONTAINMENT_REQUIRED:<reason>"; any refusal shape is fine
        # as long as no provider call happened.
        self.assertTrue(
