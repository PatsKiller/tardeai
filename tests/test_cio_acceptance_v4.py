...
def test_instance_digest_without_live_digest_fails_closed(tmp_path):
    ...
    gates = eval_g10_g12_report_formats(
        ...
        report_instance={
            ...
            "docx_sha256": d,
            "portfolio_snapshot_hash": HOLDINGS_SHA,
            "capital_plan_digest": "e" * 64,
        },
    )
    assert all(g["status"] == "FAIL" for g in gates)
    assert any("unproven" in g["reason"] for g in gates)
...
def test_unlike_prefix_family_is_not_compared(tmp_path):
    html = tmp_path / "r.html"
    pdf = tmp_path / "r.pdf"
    docx = tmp_path / "r.docx"
    h = _write_bytes(html, b"H")
    ...
    gates = eval_g10_g12_report_formats(
        ...
        report_instance={
            ...
            "docx_sha256": d,
            "portfolio_snapshot_hash": HOLDINGS_SHA,
            "capital_plan_digest": "cp_deadbeefdeadbee",
        },
    )
    assert all(g["status"] == "PASS" for g in gates)


def test_evaluate_snapshot_uses_api_plan_digest(tmp_path):
    """Same-family API plan.digest must fail G10-G12 when the report differs."""
    snap = _clean_snap()
    snap["live_capital_plan_digest"] = ""
    snap["live_decision_digest"] = ""
    snap["capital_plan"] = dict(snap["capital_plan"] or {})
    snap["capital_plan"]["digest"] = "a" * 64
    snap["report_instance"] = dict(snap["report_instance"])
    snap["report_instance"]["capital_plan_digest"] = "e" * 64
    v = evaluate_live_snapshot(snap)
    by = {g["gate"]: g for g in v["gates"]}
    assert by["G10_report_live_html"]["status"] == "FAIL"
    assert "capital_plan_digest mismatch" in by["G10_report_live_html"]["reason"]
