...
def test_gate3_freezes_candidates_at_persistence_boundary():
    freeze = ROUNDTRIP_RUNNER.index('frozen = _json_snapshot(packet)')
    persist = ROUNDTRIP_RUNNER.index('packet_id = int(original_persist(packet, *args, **kwargs))')
    snapshot = ROUNDTRIP_RUNNER.index('snapshots[packet_id] = frozen')
    ...
    assert freeze < persist < snapshot < persist_patch < execute < restore
    assert 'snapshots: dict[int, Any] = {}' in ROUNDTRIP_RUNNER
    assert 'expected = snapshots.get(packet_id)' in ROUNDTRIP_RUNNER
    assert 'snapshot_packet_ids' in ROUNDTRIP_RUNNER
    assert 'candidate_semantic_hash' in ROUNDTRIP_RUNNER


def test_gate3_verifies_exact_packet_ids_all_roles_and_live_uniqueness():
    for marker in (
        'WHERE packet_id=%s',
        ...
        'def _live_packet_ids(',
        'after_live_packet_ids',
        'live packet ids {live_packet_ids} != [{packet_id}]',
        'readback_conn.close()',
        'except Exception as exc:',
    ):
        assert marker in ROUNDTRIP_RUNNER
    assert 'except BaseException as exc:' not in ROUNDTRIP_RUNNER
...
def test_v2_numeric_normalization_cannot_conceal_a_value_change():
    ...
    assert V2._semantic_differences(1.0, 2) == [{
        "path": "$",
        "reason": "value",
        "expected": 1.0,
        "actual": 2,
...
def test_v2_never_normalizes_across_json_types():
    ...
    typed = V2._semantic_differences(1, "1")[0]
    assert typed["reason"] == "type"
    assert typed["expected"] == {"type": "int", "value": 1}
    assert typed["actual"] == {"type": "str", "value": "1"}


def test_v2_structural_comparison_stays_strict():
    assert V2._semantic_differences([1], [1, 1])
    assert V2._semantic_differences([1, 2], [2, 1])
    assert V2._semantic_differences({"a": 1}, {})
    assert V2._semantic_differences({}, {"a": 1})
    assert V2._semantic_differences("1", "2")
    assert V2._semantic_differences(True, False)
    assert V2._semantic_differences(None, False)
