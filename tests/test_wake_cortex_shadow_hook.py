...
def test_helper_flag_on_writes_agent_views_without_commitment_when_flag_off(tmp_path, monkeypatch):
    monkeypatch.setenv(CORTEX_SHADOW_FLAG, "1")
    monkeypatch.delenv(COMMITMENT_FLAG, raising=False)
    env = {CORTEX_SHADOW_FLAG: "1"}  # commitment unset/off
    out = _maybe_cortex_shadow_after_wake(
        state_root=tmp_path,
        subject_guid=SG,
        wake_id="wake-abc",
