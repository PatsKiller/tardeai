...
def test_create_when_zero_matches():
    d = decide_mirror_action(
        matching_files=[], stable_file_id=None, local_sha="abc", remote_sha="abc", readback_ok=True
    )
...
def test_reject_hash_mismatch():
    d = decide_mirror_action(
        matching_files=[{"id": "fid1"}], stable_file_id="fid1", local_sha="aaa", remote_sha="bbb", readback_ok=True
    )
...
def test_reject_readback_failure():
    d = decide_mirror_action(
        matching_files=[{"id": "fid1"}], stable_file_id="fid1", local_sha="aaa", remote_sha=None, readback_ok=False
    )
