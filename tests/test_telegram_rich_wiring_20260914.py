...
def test_payload_carries_link_preview_options():
    ...
    payload = tt._base_payload("1", "x", thread_id=None, reply_markup=None, parse_mode="HTML", link_preview_options=lpo)
    assert payload["link_preview_options"] == lpo
    assert "link_preview_options" not in tt._base_payload("1", "x", thread_id=None, reply_markup=None, parse_mode=None)
...
def test_refused_html_is_resent_readable_not_with_tags(monkeypatch):
    ...
    def post(url, payload):
        ...
        return {
            "ok": len(posted) > 1,
            "status_code": 200 if len(posted) > 1 else 400,
            "response": {"result": {"message_id": 7}},
        }

    html_text = '<b><a href="https://h.example/v3/watch/intelligence/AXTI">AXTI</a></b> &amp; <i>chart</i>'
    res = tt._deliver_text_raw(
        token="t", chat_id="1", text=html_text, parse_mode="HTML", post=post, link_preview_options={"is_disabled": True}
    )
...
def test_gateway_provider_forwards_the_preview(monkeypatch):
    ...
    seen = {}
    monkeypatch.setattr(
        sta, "_raw_send_telegram_result", lambda body, **kw: seen.update(kw) or {"ok": True, "message_ids": ["9"]}
    )
    lpo = {"url": "https://charts2-node.finviz.com/chart.ashx?t=AOUT"}
    out = ca._provider_send_telegram(body="x", kwargs={"chat_ids": ["1"], "link_preview_options": lpo}, mode="ACTIVE")
...
GOOD = {
    "symbol": "ARMP",
    "run_label": "0900",
    "scanned_at": "2026-09-14T14:08:00",
    "score": 51,
    "grade": "A+",
    "decision": "GO",
    "rvol": 19.4,
    ...
    "float_m": 11.6,
    "volume": 2_254_990,
    "catalyst": "FDA Breakthrough Therapy & label",
...
def test_go_alert_sends_the_rich_layout_with_buttons_and_chart(monkeypatch):
    ...
    assert "READ_ONLY_ADVISORY" not in text and tt.parse_mode_for(text) == "HTML"
...
def test_entry_alert_is_rich_and_keeps_the_entry_alert_words(monkeypatch, rich):
    ...
    plan = {
        ...
        "target_price": 15.5,
        "risk_reward": 3.2,
        "entry_thesis": "Holding the 20-day",
        "invalidation": "Close under 10.90",
        "proposal": {"tag": "NEEDS_CONFIRMATION"},
