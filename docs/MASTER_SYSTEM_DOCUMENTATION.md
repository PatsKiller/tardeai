...
| **Stage 2c — Stop Management (LIVE production, 2026-06-15)** | `schwab_pilot_orders`(kind) / `trade_approvals` / `stop_lifecycle` / `stop_grok_reviews` / `paper_trades` / `system_controls`(`protective_stops_enabled`) | **Protective stops live across all taxable + both Schwab IRAs** (Fidelity 401k ticket-only; no API). Committed envelope `protective_stop_policy.py` (SELL-to-close, stop<price, qty≤held, ±8% drift, ≤$250k; tamper-evidenced). Protective stops are **standing** (no ARM — `_protective_unlocked()` = policy ENABLED + `protective_stops_enabled`='true'); the canary BUY pilot still ARM-gates. **Manual + per-order 2FA** (web typed-ticker OR Telegram/email code) for every Schwab place/**Modify** (cancel-old-then-place, one 2FA)/Cancel. **Monitoring**: `stop_lifecycle_monitor.py` (lifecycle/coverage/proximity/health, `GET /api/v2/stops/lifecycle`) → card ✓ PROTECTED banner + oversized/partial warnings; **health agent** `stop_health_check.py` (SIEM+Telegram+system_health, */10 mkt-hrs); **Hermes** findings (`research_type=stop_health`/`stop_curation`); **Grok** R:R curation `grok_stop_review.py` (advisory). **Alpaca = AUTOMATIC** (`alpaca_stop_manager.py`, ratchet stops up to the R:R-optimal level via `strategy_trailing_policy`, paper-only, no 2FA, */20 mkt-hrs); all other accounts manual. **Full architecture: [`docs/brokers/stop-management-architecture.md`](brokers/stop-management-architecture.md).** |
