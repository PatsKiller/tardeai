...
|---|---|---|---|
| Ruff lint, `F` rules (undefined names etc.), line length 120, target py310 | `scripts/agent_changed_file_quality.py` (changed `.py` files; blocks if Ruff missing), pinned `ruff==0.16.2` (`scripts/lib/sop_toolchain.PINNED_RUFF_VERSION`); CI `agent-governance` installs the pin | changed Python files | `pyproject.toml` ignores `F401/F811/F841` globally, plus per-file for `tests/**` and `scripts/api_v2.py`. Ratchet file by file; don't mass-fix |
| `ruff format --check` on changed files | `agent_changed_file_quality.py` | changed Python files | Legacy unformatted files are left alone. Don't reformat a file you only touched one line of: it bloats the diff |
...
| Production schema changes need a `db-write` guard grant whose reason names the change | `bin/guard` (terminal hook; heuristic) | shell commands | Non-terminal entry points are not covered |
...
| Expected-ON units and flags | `scripts/check_expected_services.py` (read-only report) | `config/expected_services.json` | Drift: `portfolio-server`, `cio-governed-bridge` undeclared |
...
| Secrets live in Bitwarden Secrets Manager; logical names in `config/secret_registry.yaml` | `bin/guard` marks `secret` never grantable | — |
...
| Rule | Enforced by |
|---|---|
| Push only with `TRADEAI_REMOTE_PUSH_AUTHORIZED=1` + explicit operator intent; budget 1 (max 2) pushes per tranche | HOOK `pre-push` (auth + two-push budget) |
| Force-push, history rewrite | denied by Claude Code settings; `bin/guard` `git-push` scope |
...
| Independent review of sensitive paths; CODEOWNERS | **UNENFORCED** (no CODEOWNERS file) |
| Deploy = `prepare` then `promote`; verify live SHA and process `cwd` independently | `scripts/cio_phase2_exact_main_deploy.sh` (refuses a hybrid SHA; auto-rollback on failed health) + `release-write` grant |

## 10. Agent workflow (session to handoff)

1. **Start.** Run `scripts/new-worktree.sh <name>` (never work in the primary tree), then
...
6. **PR.** One push under a grant whose reason names this branch. The PR body states base/head
...
8. **Deploy verification.** `prepare` → `promote` → check `CURRENT`, the unit `cwd`, and the dev-tree SHA.
   Restart units promote doesn't bind (the CIO desk bot). Observe one natural cycle.
