...
**Status:** SHIPPED 2026-09-13 (One Source of Truth, Phase 7) · **Authority:** READ_ONLY_ADVISORY
...
falls back to `DEFAULT_ON_GAP` when a domain has none. The proposal for every domain is
`docs/implementation/sot/phase7_registry_patch.json` (the registry itself is owned by a parallel agent
and is not edited here). Highlights of the proposal:
...
Schedule proposed, not installed: `config/systemd/user/tradeai-gap-resolution.{service,timer}` (every
30 min at :07/:37, `Persistent=true`, `SuccessExitStatus=0 1`) and lane `gap-resolution-audit` in
`config/lane_registry.json`. Installing is operator-only (AGENTS.md §9.3).

## How to add a vector — registry first

1. Add the vector name to the `on_gap` chains that may use it in `config/data_source_authority.json`
   (via the patch file until the registry merges), with `cost_class`, `max_per_day`, `expected_seconds`.
2. Add the name to `gap_resolver.VECTORS` and an implementation to `DEFAULT_VECTORS`. It must: be
...
3. Add a row to the table above and a test that injects the vector's I/O and asserts the receipt.
4. A vector that is not in the registry does not run: `normalise_chain` drops unknown names.
