...
Two operator litmus tests on Telegram failed: "get back into schg" got the whole re-entry book, and a seasonality-and-sectors question was answered from DeepSeek general knowledge while the reply said cash and holdings were empty (the CIO snapshot carried $710,933 cash, sector weights and the investment policy). No reply said where its facts came from.

- **One reply chokepoint (#998).** Every operator reply passes `reply_provenance.finalize_operator_reply`: a `Sources:` line naming the stores read, `Went outside:` only when the answer left the Command Center, then the authority tail. Before, 14 of 18 reply branches sent no Sources line. House facts are read before any model call; `operator_evidence_contract` reports `MISSING_FACT` / `FALSE_EMPTY_CLAIM`; `operator_subject_resolver` binds questions to instruments (SpaceX → SPCX), and on 36 real operator questions misrouted answers went 14 → 0. A named symbol gets its own re-entry card, not the book.
- **Data gap queue reconnected (#998, operator-approved 2026-09-13).** `scripts/lib/writers/data_gap_registry_writer.py` is the only writer of `data_gap_registry` (writers 2 → 1; registry domain `data_gaps`), with one dedup rule. The desk queues real rows and names the gap ids and the resolver's next run read from the crontab, or promises nothing. `resolved` now requires the dispatched job to complete with a result row; a failed job reopens the gap and three failures abandon it. A pending with an ETA stays open to ETA + 1 h instead of a flat 2 h. The table had had no new row since 2026-05-24.
- **Agent job input caps (#998).** `watchlist_{maria,risk,steph}_flash_narrative` `max_input_tokens` 4,000 → 8,000 in `config/llm_process_registry.json`: prompts ran ~4.0–5.4k tokens, so most calls failed before reaching the model. Daily cost and request caps unchanged.
- **Rule G0 and the number check (#999).** `GROUNDING_RULE` (use only supplied facts) in every watchlist agent contract; `scripts/lib/agent_number_grounding.py` checks every number in the answer against the prompt sent and, in the default `enforce` mode, demotes an answer with ≥3 unsupported numbers that are ≥50% of those checked to `RESEARCH_MORE` below the 40% gate (`AGENT_NUMBER_GROUNDING_MODE=record|off`). A demoted result is not proof for the gap resolver. A dry run over 300 stored results flagged 3 at the default threshold — an upper bound, because stored results keep an input snapshot, not the prompt. Two checker defects found in calibration were fixed (pairwise derived changes matching an invented 27.3% by chance; negative numbers dropped). Live rate: `scripts/report_agent_number_grounding.py`.
- **Named-stock answers (#1000).** Price and 30-day change through the broker `daily_bars` projection; levels from the symbol's re-entry desk row; analysts with as-of date and age; the newest 3 research rows per type instead of the newest 60 overall (Visa's one deep-research note had been crowded out by 166 options-desk and stop-curation rows); a "What this means" line. DeepSeek Flash may summarise the brief only when every number in the summary is in the brief and symbol, close, mean target and as-of survive (`CIO_SUBJECT_FLASH=0` disables). Closing messages for a pending state the question, how long it was open, why it closed, what was missing, and retry advice from the deterministic resolver. A second `_fmt_usd` in the desk module had silently replaced the whole-dollar cash formatter; the price helper is now `_fmt_price`.
- **Chat memory per subject GUID (#1001).** `subject_memory()` reads up to 3 earlier operator questions per subject GUID in the same chat within 30 days from `operator_conversation_turns`, paired with the reply by `reply_to_message_id`, and appends "Earlier on V" plus the close move since to the subject brief and single-symbol re-entry cards (`CIO_SUBJECT_MEMORY`, `CIO_SUBJECT_MEMORY_DAYS`). New registry domain `operator_conversation` (writer `scripts/lib/inbound_identity_tagger.py`, ceiling 1). Lane `moomoo-live-read-sync` declared.
- **Ops.** `tradeai-operator-answer-quality.timer` (`scripts/check_operator_answer_quality.py`, :22/:52) installed — a promote does not install new user units. The Moomoo taxable account is open ($500 cash, 0 positions; the 2026-09-01 "account closed" crontab note was wrong); its read-only sync is re-enabled under `timeout --kill-after=30s 10m`, and the crontab edit needed the lane registry row or `check_lane_registry` failed acceptance. `tradeai-cio-telegram.service` must be restarted after a deploy that changes desk code — promote restarts `portfolio-server` and the health agent only. All three rules are now in `AGENTS.md` §9.3 and §10. `[VERIFIED]` 2026-09-13 23:54 ET, `systemctl --user list-timers`: the six monitor timers (served-copy split, expected services, data plausibility, data-source health, gap resolution, operator answer quality) are scheduled; data plausibility had not fired yet.
- **Known and named, not fixed.** `check_operator_answer_quality.py` still flags `PENDING_NEVER_CLOSED` at a flat 2 h while the desk keeps an ETA pending open to ETA + grace. Attention replies on the converse path scan an empty office. Only desk turns persist `reply_provenance` durably. The Moomoo sync receipt `data/runtime/sync_receipts/moomoo_taxable_live.json` did not exist at 23:57 ET Sunday; the first natural run is Monday 09:00 ET. In flight, not merged: `feat/synthesis-prompt-budget`.

## 2026-09-13 — One Source of Truth: one declaration per domain, one writer per store (PRs #992–#997, `36c351c23` → `c00e23eda`)

MATURITY_IMPACT: which store, writer, provider and read path is authoritative is now a program-readable registry that fails the build when violated. Execution posture UNCHANGED — labelling, monitoring and write-path consolidation only; no position, balance, order or stop changed. Registry `config/data_source_authority.json` · rendered `docs/SOURCE_OF_TRUTH.md` · rule `AGENTS.md` §7A · map `docs/MASTER_SYSTEM_DOCUMENTATION.md` §5.7 · audit `docs/implementation/sot/DOCS_AUDIT_20260913.md`.

Three incidents in one week had one root: nothing a program could read said what was authoritative. A 1–5 analyst-rating column held 10-year performance for five months (Finviz positional maps); the served state tree and the tree the 344 cron producers write to were two directories for 18 days; the catalyst-news chain ran with four dead slots out of six.

- **The desk stops promising what it cannot deliver; the split state tree is monitored (#992).** `is_answerable()` refuses a market question with no resolvable instrument at once (the SpaceX ask had sat open 72 minutes and would have sat forever); an answerable pending is retracted at 2 h. `check_served_copy_split.py` checks directory identity, not file age — LINKED is the only OK — hourly at :42, read-only.
- **The registry and its gate (#993).** 17 domains × {store, single writer or `UNCONSOLIDATED` + ceiling, cadence, stale window, projection, same-question backups, retired, `no_coverage`}; `check_data_source_authority.py` (`RETIRED_CALL_SITE`, `UNDECLARED_PROVIDER`, `WRITER_COUNT_ROSE`, `DIRECT_READ_ROSE`, …) in acceptance and CI; AGENTS.md §7A rendered. Finnhub, Polygon, FMP and NewsAPI retired: 36 call sites → 0; `polygon_source.py` archived with a manifest row.
- **Phases 3–7 (#994).** Health rows decay to `unknown` outside their window; the broker envelope (`as_of / age_hours / source / stale / gap`) on every projection, dead desks answer `gap: no_producer`; Brave denials spill to SearXNG through the registry chain; accounts render `LIVE / STALE / SERVICE_DOWN / NO_API_MANUAL`, never `$0` for absent; `gap_resolver.resolve()` walks each domain's `on_gap` chain, free before metered before paid, receipted, dry-run unless `GAP_RESOLVER_LIVE=1`. Registry at 24 domains (26 after #998 and #1001).
- **Weekend-aware decay (#995).** A source whose every scheduled caller runs Mon–Fri measures age in weekday minutes (`weekday_only_for`); a domain's `stale_after_hours_closed` applies when the market is shut; six missing scheduled callers declared; `catalyst_news` window 12 h → 18 h. Without it the hourly audit would have interrupted the operator eight times before Monday's first cron.
- **One write module per store and the operator grant (#996).** `scripts/lib/writers/` holds the only INSERT/UPDATE SQL for `market_quotes` (3 → 1 writers), `ticker_prices` (4 → 1), `news_articles` (16 → 1), `symbol_profiles` (8 → 1), `watch_directives` (18 → 1) and `hermes_research_intelligence` (32 → 1); golden tests prove each issues the legacy statement and resolves the same subject GUID. Registry → `DataSourceAuthority@v2`: every provider and domain carries the operator's `approval`; `UNAPPROVED_SOURCE` fails an ungranted row; §17 gains adding, replacing or retiring a data source or a writer of an authoritative store.
- **An alert written for the operator (#997).** The data-source-health alert says what each source feeds, what is wrong, its next scheduled run in local time and an Action line, grouped Failing / Stale health row / Waiting on first report, with escalation stated; cron strings and paths move to a trailing "Details (for the engineer)" block.
- **Known and named, not fixed.** `watch_directives` live CHECK admits `expired` but not `proposed`; `news_articles.relevance_score` mixes 0–1 and 0–100; proposed migrations in `docs/implementation/sot/phase9_*_notes.md`; `CALLER_DAILY_CAP` Brave denials do not spill (operator decision).

## 2026-09-04 — The header's clocks were one field, and its counts dropped twelve rows (`a7c550d1d` → PR #857)

MATURITY_IMPACT: the Command Center header stops asserting a single date over an aggregate it cannot date. Two producer contracts gain named clocks and a value-coverage measure; the setup taxonomy gains the two classes it was folding into "unclassified". Execution posture UNCHANGED — read-only diagnosis, no broker authority touched.

An operator capture taken against the release deployed hours earlier showed the header still contradicting itself. The previous cycle had fixed labels; the producers underneath still emitted one field where there were several truths, so better wording had nothing truer to say. Both defects were fixed at the producer.

- **The clocks were one field.** `PortfolioAggregate@v1` published a single `observation_time` per account and dated the whole aggregate by it. That field is the POSITION observation — when share counts were last seen — not when the money was valued. On the live book `schwab_rollover_ira` carried `as_of 2026-07-17` (shares), `last_repriced 2026-09-04` (valuation), `reported_total_as_of 2026-04-30` (custodian's own total) and 90% of the book's value. So the header read `data_as_of 2026-09-03` — the newest position observation, belonging to a **$5,000** account — over a **$1.28M** total priced from quotes observed 2026-09-04 13:30 ET, beside a book panel reading `session 2026-09-04`. Every number was true; together they were a lie. **`PortfolioAggregate@v2`** publishes each clock under its own name and adds `coverage.at_newest_pct`: the share of aggregate VALUE the headline date is entitled to describe. On the captured book that is **0.4%**. v1 field names are retained as exact aliases.
- **Today's P&L had no date of its own.** Nothing in the payload said which session the day-change belonged to, so the tile borrowed the position clock and stamped a 2026-09-04 intraday figure `2026-09-03`. **`TodayPnl@v1`** gives it its own session (the session its marks were struck in), its calculation time, and coverage that names the two linked accounts contributing nothing rather than reporting "4 contributing".
- **Twelve rows had no name.** `48 classified / 60 scanned / 0 excluded`. 48 + 0 = 48. The missing 12 were `MANUAL_REVIEW` — a disposition the scanner sets deliberately (it has a `manual_review_required` column) for setups scoring 30–38. They were never undecided and are emphatically not NOGO, but the taxonomy had no token for them so they landed in `unclassified`, which asserts the pipeline failed. `review` and `error` join the taxonomy; `classified` stays GO+WAIT+NOGO because an escalation is not a classification; any residual renders as `N UNACCOUNTED` rather than being left for the reader to subtract.
- **`scanned` disagreed with itself, 60 vs 49.** The ticker query spans two days and collapses to one row per symbol via `DISTINCT ON`, while the current-run filter matched on **run_label alone** — so a symbol scanned in *yesterday's* 0900 run and absent from today's was counted into today's. That, not the taxonomy, is what pinned `count_integrity` at PARTIAL. Run identity is `(run_date, run_label)`; the filter now uses both.
- **Counts that named no scope.** The operator inbox rendered "82 items" above a list of 80 — `count` was the whole queue, `items` a page of it. Home's pending row carried a red `P0` chip in front of `pending_count`, reading "P0 12 proposals awaiting review" while the inbox beside it said "P0 7" from `p0_count`. Degraded quotes said `DEGRADED (price_cache_nav(1))` — something is wrong, nothing about how much. All three now state their scope and coverage.
- **Two assertions encoded the defect.** One required both money tiles to carry `asOfLabel: 'data_as_of'`; `setupRunSummary`'s expected population string omitted a non-zero `unclassified_count`. Both passed while the header was at its most misleading. Inverted, with a negative control proving the new rails trip on the defect rather than on their own setup. `scripts/test_metric_strip_labels.mjs` was runnable but **no workflow invoked it**; `cc-header-truth-ci` now gates every path asserted.
- **Evidence.** `tests/fixtures/header_truth/` holds both contradictions verbatim from the production payloads — the Sep-3/Sep-4 clock pair and the 48/60/0 partition — pinned by 13 tests registered under the `cc_header_truth_v2` gate.
- **Known and named, not fixed.** `quote_selection` does not yet publish per-symbol coverage counts, so the header renders the coverage fragment only when the producer supplies it. Three of six accounts still carry no position observation at all; they are reported as undated rather than dated by inference.
...
- **Loop closure.** The two lineage arcs had zero envelope overlap, so neither could satisfy the per-envelope completion predicate: the research arc had a checkpoint and never a notification decision, the CIO arc had the notification and never a checkpoint. Each now finishes its own record — research settles its notification stage as NOT_REQUIRED with a reason, CIO runs persist the `OutcomeCheckpoint@v1` they never wrote. The predicate is untouched and two tests assert a workflow still cannot complete on half the evidence (#560).
- **Checkpoint → outcome (P0).** `process_due_checkpoint()` and `persist_observation()` had existed unused since the loop landed, so 183 checkpoints accumulated with 50 past due and **none ever resolved**. Resolver shipped. **The first working version resolved 43 of 50 and was wrong**: 37 were `HOLD_CASH` decisions about the cash sleeve being priced against `CASH`, a real listed equity (Pathward Financial) that resolves in the identity registry as CONFIRMED — only the recommendation distinguishes them. `price_resolvable()` now refuses non-security subjects with a recorded reason; the honest count is **6 resolved, 44 refused** (#561).
- **Evidence domains, second layer.** Fixing the first four exposed four more that no run had ever got far enough to report. `analyst_actions` resolved a collector name that never existed; `catalysts` resolved one that required arguments the snapshot never passes; `cash_buying_power` had a working collector and a **date-only** `as_of` against a 1-hour threshold (the `portfolio` defect from #557, same file); `open_cio_actions` needed nothing — an earlier probe of mine omitted `action_ledger` and misreported it (#562).
- **Outcome → lesson.** 1,617 lessons and 1,467 applications existed with **zero** referencing an outcome — the system was learning from the advisory KB, not its own results. Linked. **The obvious implementation would have stamped a lesson SUPPORTED at n=5** from five records of one event (same symbol, recommendation, date, horizon); `MIN_LESSON_SAMPLES` is exactly 5. Observations now group by independence and contribute one sample each, with correlated siblings recorded but uncounted. First lesson: PROVISIONAL, 1 independent sample of 5 observations (#563).
- **Snapshot integrity.** `cio_portfolio` imports `DomainEvidence` from `lib.*` while the snapshot imports it from `scripts.lib.*` — two distinct class objects, so `isinstance` was False for **8 of 18 broker collectors**, each dying with "Unexpected collector return type" and then being relabelled "not yet collected" by a fallback that overwrote the real reason. Duck-typed; usable domains **14 → 19** (#563).
- **Ops.** CIO wake dispatcher moved from `*/5 9-16 * * 1-5` to `*/5 * * * *`. 47 research/catalyst jobs already ran 24/7, but the component that turns evidence into a run did not — a catalyst landing Thursday evening or a weekend filing sat unprocessed until Monday.
- **Known and named, not fixed.** `holdings_detail` STALE with `positions_built_at` at 2026-07-17; `defense_stops_protection` blocking `RISK_OR_STOP_EVENT`; `reentry` still has no zero-arg collector (pinned by guard test). 44 of 50 due checkpoints were not price-resolvable, so the learning loop currently cannot evaluate most of what the CIO decides — a design question, not a bug.

## 2026-08-27 — Persistence wiring: identity spine, lineage, memory, evidence gate (6 PRs, `6124ee46` → `5bd09f37`)

MATURITY_IMPACT: identity and memory layers moved from built-but-dark to wired on live paths; a 17-day silent outage of the CIO run pipeline closed. Execution posture UNCHANGED (everything here is `READ_ONLY_ADVISORY`; no broker authority added). Full record: `docs/architecture/cio/PERSISTENCE_WIRING_2026-08-27.md`.

- **Identity.** `schwab_adapter` stopped discarding the broker CUSIP it received on every position sync (#551), and the registry stopped discarding it a second time one layer down — a CONFIRMED entity now stores the `identifiers{}` and `identity_basis` that confirmed it, so the status is auditable and the GUID re-derivable (#554). Registry scope widened from `status='active'` to every status except `removed`, plus re-entry candidates, traded-ever and the symbol master: **383 → 5,277 live entities**, closing 43 re-entry and 117 traded symbols that had no identity at all (#555). The sweep returned **4,997 identifiers from 5,244 symbols (95.3%), zero fetch failures**, taking CONFIRMED entities **17 → 5,014**; folding it in moved no existing GUID and left all 5,002 superseded ids resolving forward. CUSIPs swept from Schwab's instrument reference, which returns them for names never held; Alpaca `/v2/assets` **has no `cusip` field at all** across 14,281 active US equities and cannot close this (#555).
- **Lineage.** Envelope subjects now resolve against the registry on the write path rather than only when a producer passed an explicit payload — which the CIO arc never did. `subject_guid` **0 → 58 of 97**, `entity_type` 58 SECURITY. `resolve_entity` had treated a declared `UNRESOLVED` as authoritative, keeping stale envelopes unresolved even once the registry could answer. Registry parse cached on `(mtime_ns, size)`: 18.2 ms → 0.019 ms per envelope. Historical envelopes re-stamped append-only, 1,142 → 1,200 rows, 0 removed (#556).
- **Evidence gate (P0).** 54 of 55 CIO runs blocked at `HEALTH_CHECK` continuously since 2026-08-10, so none reached synthesis. The fail-closed gate was correct every time; four unrelated wiring bugs starved it: two stores never passed to `CIORunWorker`, a collector resolved by a name that never existed (miss swallowed by a bare `except`), and portfolio freshness stamped from `as_of` — a date-only field that can never satisfy a 12h threshold past noon. Domains AVAILABLE **7 → 11**. The gate itself is untouched and a test asserts that (#557).
- **Memory.** Cognitive memory anchored on the durable spine: 441 live records carried ticker strings and no `subject_guid`. Now 428 single-entity records carry one; multi-symbol records carry `subject_guids` and no single subject; unregistered symbols are named under `unresolved_symbols` rather than dropped. Memory reads the registry and never mints (#558).
- **Ops.** `PipelineLiveness@v1` — alerting on *absence of throughput* rather than error-log parsing, because the 17-day outage produced no errors: the gate was working correctly and saying so. `STARVED` (work in, nothing out) is separated from `QUIET` (nothing to do) so the monitor does not fire on idle windows and get muted. Read-only; `--fail-on-finding` for cron/CI gating. Alert-channel wiring is a separate explicit decision and is not done (#558).
- **Known and named, not fixed.** `analyst_detail` and `reentry_decision_desk` have no zero-arg collector and `catalyst_record.get_catalyst_record` requires args the snapshot never passes — same silent-failure shape, none REQUIRED for currently blocked run purposes, all pinned by a guard test so a new instance fails CI. The CIO arc still records no checkpoint (arc overlap is 0, so the completion predicate remains unsatisfiable); `memory_m2_v2` still has zero non-test consumers; 0 of 441 memory records carry `valid_from`/`tx_from`.
