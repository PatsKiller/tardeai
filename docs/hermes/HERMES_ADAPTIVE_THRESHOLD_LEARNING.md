...
- **Proposal history** (collapsible) — last 10 `decided_proposals` with `impact_narrative` (plain-language 14d metric deltas), `evaluation_outcome` (verdict → recommendation) when `--evaluate` has run, plus recent audit tail
