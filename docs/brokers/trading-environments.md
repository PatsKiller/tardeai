...
**Status:** CANONICAL taxonomy (2026-07-21) · **Implementation:** paper live; personal/IRA **not** wired  
...
silently when live **Paca personal** or **Paca IRA** credentials are added if:
...
| `paca_personal` | Alpaca **live taxable / personal** brokerage | Real | `api.alpaca.markets` | **NOT IMPLEMENTED** |
| `paca_ira` | Alpaca **live IRA** (Traditional/Roth) | Real retirement | `api.alpaca.markets` (account-scoped) | **NOT IMPLEMENTED** |
...
| *(future)* `paca_personal` | `paca_personal` | Proposed |
| *(future)* `paca_ira_traditional` / `paca_ira_roth` | `paca_ira` | Proposed split if both opened |
...
environments:
  ...
  paca_ira:
    ...
    trading_base_url: https://api.alpaca.markets
    account_keys: [paca_ira_roth, paca_ira_traditional]
    credentials_env:
...
| `ALPACA_MODE=paper` | `BROKER_ENV=paper` | `BROKER_ENV=paca_personal` | `BROKER_ENV=paca_ira` |

Keep reading legacy names during transition; **fail closed** if live URL + paper-only code path.

## 6. Code ownership by env

| Concern | paper | paca_personal / paca_ira (future) |
|---------|-------|-------------------------------------|
...
| UI routing labels | `paper_auto` | `live_paca_personal` / `live_paca_ira` |
...
| **Paca personal** | Live Alpaca individual taxable brokerage (API) |
| **Paca IRA** | Live Alpaca Traditional or Roth IRA (API-enabled per Alpaca 2025–2026 product) |
