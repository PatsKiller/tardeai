...
| **Revision** | **R6.3 — 2026-08-18T21:49Z** (loop closure producers; **not R7**) |
...
> **Not R7.** Overlay is not empty. Autonomous-loop / deep-research were not re-run to green this hour. No desk consumer path. Do not promote a docs-only follow-up as if it were this code.
...
| Hermes autonomous-loop | **PATCHED_NOT_REPROVEN** | last fail Aug 17 MU/STLD timeout; host unit timeout/budget patched |
| Hermes deep-research-local | **PATCHED_NOT_REPROVEN** | last fail Aug 18 SSL; live script now reconnects |
...
| gap | status | note |
|---|---|---|
| G-HER-01 | **patched, not re-proven** | overnight units not run this hour |
