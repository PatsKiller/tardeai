...
| **M3-S6** | T1 upgrade — Lee-Ready `TFI`, effective spread, Kyle's λ, VPIN | Tier parity measurable (G7) | ⛔ **BLOCKED by S5.5**: real-time consolidated (SIP) trades/NBBO are UNAVAILABLE (`T1_IEX_ONLY`). Do NOT build TFI/spread/λ/VPIN until an eligible consolidated feed exists |
