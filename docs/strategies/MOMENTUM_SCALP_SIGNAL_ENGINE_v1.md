...
| **M3-S5.5** | **Multi-source observation fabric + provider entitlement proof.** Provider-neutral bounded-concurrent acquisition, normalized provenance/freshness/entitlement, deterministic source arbitration; trigger engine + external behavior unchanged | Source matrix proven; Alpaca real-time SIP/IEX unambiguous; Yahoo/Schwab/Moomoo roles explicit; bounded concurrency + failure isolation tested; no silent fallback/averaging; T0 results reproducible; engine still shadow-only | 🟡 **PREPARED (draft PR)** — findings: `docs/_findings/M3_S5_5_PROVIDER_ENTITLEMENT_2026-07-27.md`. **Alpaca T1 = `T1_IEX_ONLY`** (real-time consolidated SIP unavailable) |
| **M3-S6** | T1 upgrade — Lee-Ready `TFI`, effective spread, Kyle's λ, VPIN | Tier parity measurable (G7) | 🟡 **PREPARED, GATED (feed pending)** PR — `scalp_t1_metrics.py` (pure, fixture-tested) + `scalp_t1_gate.py` (refuses unless `t1.enabled` AND entitlement==`SIP_REALTIME`; IEX-only/delayed FORBIDDEN). `t1.enabled: false`. **Activation:** procure a consolidated feed (Alpaca Algo Trader Plus / Polygon Developer+ / Moomoo T2) → re-probe → set `SIP_REALTIME` + `t1.enabled: true`. Cannot run on the current IEX-only entitlement |
| **M3-S7** | Enable Telegram tiers. Alerts only, still no proposals | G4 + G6 satisfied | ✅ **SHIPPED** — `scalp_alert_emitter.py` (§3.4 lane→tier via `alert_dispatcher`: IGN_45→INFO/dashboard, IGN_60/ACCEL/75→ALERT tagged "UNGATED — NOT A PROPOSAL"; per-run cap 8; dispatcher dedupes once/day/symbol + 15/hr). `notifications.emit: true`. **Alerts only — no proposals** (IGN_75→ALERT, not a proposal). Replay always dry; live-only sends. **G4/G6 evaluated as the sample accrues** (enabled pre-validation; instantly reversible) |
| **M3-S8** | Weight refit, lock, restart sample | Locked weights committed with hash | 🟡 **TOOL PREPARED, GATED (sample pending)** — `scalp_weight_refit.py`: G1 gate (≥100 fires/≥15 sessions) + pure logistic refit + deterministic held-out split + G3 monotonicity check + hash-locked record. **REFUSES on the current sample** (6 fires/1 session → `WeightRefitBlocked`). Weights stay at v1 priors until G1 accrues. Runs ONCE then locks + sample restarts |
| **M3-S9** | Proposal emission → AUTO_PAPER | G1–G7 all satisfied; operator sign-off | ⏳ pending |
| **M3-S10** | T2 layer, contingent on moomoo M0/M1 delivering API depth | Bolts onto §5 without touching S1–S9 | 🟡 **CAPABILITY PREPARED (not wired)** — `scalp_t2_metrics.py` (book imbalance / quoted spread / Stoikov micro-price / depth, pure) + `market_observations/moomoo_t2.py` (order-book provider through the Stage-0 client boundary; **fail-closed → SCAFFOLD_ONLY** on this host, OpenD not configured). **CONSERVING arm-on-demand:** `ArmedSubscriptionManager` subscribes L2 only while a symbol is armed (seconds/minutes from trading), hard `max_armed` budget (never evicts silently) + TTL auto-disarm. **ARM NOW WIRED** to the trigger FSM: the shadow logger arms a symbol's L2 while its FSM state is `ARMED` (disarms otherwise), live-only, `t2.arm_from_trigger:true`, cross-cron state persistence. Still fail-closed (SCAFFOLD_ONLY until OpenD + L2 entitlement) → no real subscription/cost yet |
