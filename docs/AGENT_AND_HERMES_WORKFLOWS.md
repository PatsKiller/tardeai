...
**Status:** Canonical workflow reference · Validated 2026-06-03 · agent grounding, job input caps and gap proof updated 2026-09-13
...
**Rule G0 and the number check (2026-09-13, PR #999).** Every watchlist agent contract (Maria, Steph, risk, tax, full chain) and Maria's one-pass prompt carry `GROUNDING_RULE` from `scripts/lib/cio_agent_contract.py`: every number stated must come from the supplied context or simple arithmetic on it; a missing fact goes in `data_i_doubt`, never an estimate. After parsing, `scripts/lib/agent_number_grounding.py` looks up each number in the summary, narrative, evidence and next action in the prompt actually sent (rounding at the precision written, 0.5%, percent/fraction forms, changes from the current price; counts, dates, model names, 401k-style names and stated confidence are skipped). Mode `AGENT_NUMBER_GROUNDING_MODE`: `enforce` (default) demotes an answer with ≥3 unsupported numbers that are ≥50% of those checked to `RESEARCH_MORE`, confidence 0.39 (below the G4 gate), reason `UNGROUNDED_NUMBERS`, original kept in `number_grounding`; `record` flags only; `off` disables. Live rate: `scripts/report_agent_number_grounding.py`. Calibration over stored results is an upper bound — stored results keep an input snapshot, not the full prompt.

**A result is proof only when it is grounded (PR #998).** `data_gap_resolver.py` dispatches a Maria research job for `missing_catalyst` / `explicit` gaps and leaves the gap `enriching`; `verify_dispatched()` marks it `resolved` only when the job completed with a result row that was not demoted for `UNGROUNDED_NUMBERS`. A dead job reopens the gap; three failures abandon it. See `MASTER_SYSTEM_DOCUMENTATION.md` §5.5.

**Flash narrative input caps (2026-09-13, PR #998).** `watchlist_maria_flash_narrative`, `watchlist_risk_flash_narrative` and `watchlist_steph_flash_narrative` carry `max_input_tokens` 8,000 in `config/llm_process_registry.json` (was 4,000). Prompts run about 4.0–5.4k tokens, so most Maria, Steph and risk flash calls failed on the cap before reaching the model. Daily cost and request caps were not changed.
...
- **2026-09-13** — Rule G0 use-only-supplied-facts in every watchlist agent contract with an enforced number check (`agent_number_grounding`, PR #999); flash narrative `max_input_tokens` 4,000 → 8,000 for Maria, Steph and risk (PR #998); data gaps dispatched to agent jobs resolve only when the job completed with a grounded result row (PR #998). Operator-desk (Telegram) reply routing is documented in `docs/OPERATOR_REPLY_ROUTING.md`, not here.
