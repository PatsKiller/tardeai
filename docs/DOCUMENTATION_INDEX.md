...
| `docs/ALERT_INCIDENT_RESOLUTION.md` | **Alert incident resolution** — the two alert planes, why `resolving` must be caller-supplied, the chain from producer to `status='resolved'`, and what still does not close (added 2026-09-21) | Active |
...
| `docs/OPERATOR_REPLY_ROUTING.md` | **Operator reply routing** — every free-text path to a sent reply (poller, CIO bot, WhatsApp, pending follow-ups), what each reads, which model it may call; the Sources / Went outside / authority-tail contract and its single chokepoint (`reply_provenance.finalize_operator_reply`) | Active (2026-09-13) |
