# Replacement: AgentCollaboration.tsx — Agent Collaboration, RACI & Outcomes

- **Target:** `apps/command-center-v2/src/pages/AgentCollaboration.tsx`
- **Original SHA256:** `db4962d40737f385084981ac5a3a8ea033182267257172f14295663470958c01`
- **Replacement timestamp:** 2026-05-24
- **Version:** 3.0 — Four-tab layout, collaboration flow, RACI map, outcome quality, inferred timeline
- **Design summary:** Full rebuild with 4 tabs (Missions, Collaboration Flow, RACI Map, Outcome Quality), collaboration flow derived from agent_network handoff history, per-mission inferred timeline, owner-level RACI fallback (labeled), observable quality signals, and honest missing-data messaging throughout.
...
- [ ] Top scorecard: Needs John, Blocked, Waiting on Agent, Stale, System Trust (all clickable, compact)
- [ ] 4 tabs: Missions | Collaboration Flow | RACI Map | Outcome Quality
- [ ] Missions tab: status + task type + time filter rows
- [ ] Active filter indicators with clear buttons
- [ ] Two-pane: Mission Queue (42%) + Inspector
- [ ] Inspector Section A: Mission Summary
- [ ] Inspector Section B: Collaboration Timeline (inferred, labeled)
- [ ] Inspector Section C: RACI via RACIMatrix (owner-level fallback, labeled)
- [ ] Inspector Section D: Agent Contributions (AgentChip + showRole, clickable)
- [ ] Inspector Section E: Blockers / Staleness (red/orange cards)
- [ ] Inspector Section F: What John Should Do (green card + ActionButton)
- [ ] Inspector Section G: Mission Items (thread list)
- [ ] Inspector Section H: Stats footer
- [ ] Collaboration Flow tab: grouped flows from agent_network
- [ ] RACI Map tab: collapsible per-agent sections
- [ ] Outcome Quality tab: observable good/bad signals + missing fields
- [ ] All StateCard tiles filter missions
- [ ] AgentChip clickable everywhere
- [ ] StatusBadge on mission cards clickable
- [ ] Same API: useApi('/api/v2/agent-collaboration')
- [ ] RACI via useState + fetch (dynamic agent param)
- [ ] RACIMatrix imported from existing component
