# Approvals.tsx Replacement

- **Target**: `apps/command-center-v2/src/pages/Approvals.tsx`

## Changes

- Inline urgency pills replaced with `StatusBadge` (mapped: urgent->blocked, normal->warning, low->unknown)
- Inline priority pills in Decision Tasks replaced with `SeverityBadge` (mapped: urgent->critical, high->high, normal->medium, low->low)
- Approve/Reject buttons replaced with `ActionButton` (children pattern, variant=primary for approve, variant=danger for reject)
- Quick nav buttons replaced with `ActionButton` variant="ghost"
- Status filter chips use `ActionButton` with variant switching
- "Full Context" button uses `ActionButton` variant="secondary"
- Decision history decision pills replaced with `StatusBadge`
...
- All supporting links preserved
...
- All API endpoints preserved:
  ...
  - `/api/v2/approvals/states` (30000ms poll)
  - `/api/v2/tasks` (30000ms poll)
  - POST `/api/v2/approvals/decision`
- All interfaces preserved (JohnTask, TasksData, RiskPos, Evidence, RiskContext, QueueItem, ApprovalData, DecisionItem, HistoryData)
- handleDecision callback preserved exactly (POST /api/v2/approvals/decision with queue_id, decision, note)
- handleDrawerDecided callback preserved
- All filtering logic preserved (statusFilter, statusMap, filteredQueue)
- Decision summary rendering preserved
- Evidence pack rendering preserved
- Risk context for STOP_REVIEW preserved
- Risk context for ALLOCATION_REVIEW preserved
- Impact comparison (If Approved/If Rejected) preserved
- TaskDetailDrawer and ProposalDetailDrawer preserved
- No new approval actions added
- No existing approval gates bypassed
- Notes/rationale requirement preserved
