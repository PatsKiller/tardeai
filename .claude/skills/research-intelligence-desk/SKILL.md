...
## 5k. DEFENSE DESK v4 (EXECUTED 2026-07-18 — see docs/architecture/DEFENSE_DESK_V4.md)

Commits 0a16274b..70a58794+. Operator graded v3 4/10 (core book invisible, no dollars/levels, empty radar rows, no step-out memory) → structural 8. **WS-L**: config/fund_lookthrough.json (factsheet weights, SPCX/DIVI/BND=none ~$90K stated) → EFFECTIVE book weights in engine rows w/ decomposition — **tech 24.1% effective (5.1% direct) LAGGING, Industrials 12.5% LAGGING, both above hedge trigger v3 never saw**; book-map sectors are FINVIZ NAMES — canonicalize (V $119K was invisible to Financials); materiality max($2K, 0.15%) → 11 scraps = ONE cleanup card; stances for all 18 ≥$10K positions incl. HOLD (SCHG $266K TRIM-WATCH 65% weak weight). **WS-RT** rotation_round_trips: advised→stepped_out(trade_transactions ingest 12h-lag OR one-tap POST /defense/round-trips/confirm)→rollback_open (conditions AT exit: sector-exit-2close/50DMA-reclaim/60d)→closed vs-held scored; wash-sale 31d deterministic + ANY-account/IRA-permanent warning, basis-blind until Cost Basis export, NEVER suppresses rollback; sqlite-backed lifecycle test. **WS-CARD**: field guard now requires levels(price/entry/stop) on actionable groups; per-account $ bands; CC concrete strikes from cc_call picks (0.15-0.38Δ 18-50DTE best-fit .28; CSCO 08-21 $125C est $282 caps +11.7%); chain universe counts UNIQUE symbols (dup V/SCHD rows ate slots — CSCO missing). **RADAR/BOARD**: radar = ONE line until ≥1 non-baseline row (n stated); chipScope.mjs unit-tested IN npm build — ranks scoped to rendered list, new-dots, callout separates improvement from deterioration. **Operator mid-session**: freshness strip + REFRESH ALL → detached defense_refresh_job.py (flock, 4 steps, status JSON polled; industries WITHOUT --close — cron owns state). GOTCHAS: git-checkout killed uncommitted work AGAIN (2nd time); self-score 8/10 structural — remaining points accrue on calendar (shadow ~Jul 31, OI n=1→20d, outcomes empty, options_level, basis export).
