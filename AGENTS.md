...
### DO NOT set SO_LINGER=0 on the server socket (2026-08-02)

`SO_LINGER` with `l_onoff=1, l_linger=0` tells the kernel to discard the TCP send buffer
immediately on close (RST instead of FIN). This setting is **inherited by every accepted
client connection**, not just the listen socket.

When the server closes an HTTP connection after sending a response, the kernel discards
any unsent data in the socket buffer. For large static files (e.g. a 4 MB React JS bundle),
the kernel can only buffer ~1–2 MB before the application's `wfile.write()` blocks. If
`SO_LINGER=0` is active, the connection RSTs before all bytes are transmitted, and the
browser receives a **truncated JavaScript file**. React never mounts. The page stays blank.
No error message, no console warning — just a white screen.

**Symptoms**: Blank page, varying download sizes for the same static file on each request,
`curl` showing fewer bytes than `Content-Length` header promises.

**Fix**: Remove `SO_LINGER` entirely. Instead:
- Set `request.settimeout(30)` on accepted connections to bound hung I/O
- Bounded semaphore (`DASHBOARD_MAX_CONCURRENCY`) prevents thread exhaustion
- `finish()` returns DB connections to the pool
- `wfile.flush()` after every `wfile.write()` ensures Python-level buffers drain

CLOSE-WAIT buildup is addressed by the request timeout + concurrency bound, not by
