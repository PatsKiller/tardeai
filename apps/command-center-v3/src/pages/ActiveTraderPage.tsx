  ...
  return (
    <section className="at-panel at-trade-card" aria-labelledby="active-trade-title">
      <header className="at-panel__header at-trade-card__title-row">
        ...
        <div className="at-inline"><Chip>{signal.lane} lane</Chip><Chip tone="warning">{signal.mode.replace(/_/g, ' ').toLowerCase()}</Chip></div>
      </header>
...
export default function ActiveTraderPage({ signals, accounts, onOpenStrategies }: Props) {
  // Reference/preview posture: until live signals are wired, fall back to clearly-LABELLED sample data
  // (never presented as live). Accounts use the real posture matrix from the API when it provides one.
  const sample = !signals || signals.length === 0;
  const queue = sample ? MOCK_QUEUE : signals;
  const acct = accounts && accounts.length ? accounts : MOCK_ACCOUNTS;
  const [selected, setSelected] = useState<ScalpSignal>(queue[0] || MOCK_SIGNAL);
  const [routing, setRouting] = useState(false);
  const sorted = useMemo(() => [...queue].sort((a, b) => b.ign - a.ign), [queue]);
  return <main className="active-trader-page">
    <div className="active-trader-page__intro"><div><h1>ActiveTrader</h1><p>Evidence-first momentum-scalp review. Manual paper testing only; no automatic or live order path.</p></div><div className="at-inline"><Chip>{selected.session}</Chip>{sample && <Chip tone="warning">REFERENCE SAMPLE — no live signals</Chip>}<Chip tone="warning">NO LIVE ROUTING</Chip></div></div>
    ...
    {routing && <AccountAllocationModal signal={selected} accounts={acct} onClose={() => setRouting(false)} />}
  </main>;
}
