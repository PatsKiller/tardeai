...
export default function ManualTosDesk() {
  ...
  return <div>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,minmax(105px,1fr))', gap: 8, marginBottom: 12 }}>{(['ALL','PROPOSAL','WATCHLIST','DRAFT','READY','GENERATED','BROKER_OBSERVED'] as const).map(f => { const n = f === 'ALL' ? wrapped.length : wrapped.filter(x => x.r.source === f || x.state === f).length; return <button key={f} onClick={() => setFilter(f)} style={{ ...btn(filter === f ? C.blue : 'var(--bg2)'), padding: '8px 10px' }}>{f.replace('BROKER_OBSERVED','LINKED')} · {n}</button> })}</div>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}><div style={{ padding: 12, border: '1px solid var(--border)', borderRadius: 8, background: 'var(--bg1)' }}><b>Lifecycle</b><div style={{ fontSize: 10, color: C.dim, marginTop: 4 }}>Candidate -> setup generated -> broker activity observed. No local button creates truth.</div></div><div style={{ padding: 12, border: '1px solid var(--border)', borderRadius: 8, background: 'var(--bg1)' }}><b>Read-only recon</b><div style={{ fontSize: 10, color: C.dim, marginTop: 4 }}>{lastRun ? `Last run #${lastRun.id} · orders ${lastRun.orders_seen} · matched ${lastRun.matched} · mismatched ${lastRun.mismatched} · ${lastRun.status}` : 'No recon run yet'}</div></div></div>
