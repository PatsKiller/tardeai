            ...
            {pageItems.map((it: any) => {
              ...
              return (
                <div key={it.id} onClick={() => onDrill({ title: `${it.symbol}${it.hermes_rank != null ? ` — Hermes #${it.hermes_rank} (${it.hermes_composite_score})` : ''}`, subtitle: `${it.origin_system ?? it.source ?? ''} · ${it.status}`, endpoint: `/api/v2/hermes/intel/${it.symbol}`, rows: [a ? { ...it, setup_advisory_note: a.note, setup_advisory_flag: a.advisory_flag, current_rsi: a.rsi, rsi_band: a.band } : it] })}
                  ...
                  {it.cio_narrative && <div onClick={e => e.stopPropagation()} title={it.cio_narrative.replace(/^\[DUAL-CONSENSUS\]\s*/, '')} style={{ fontSize: 10.5, color: TEXT2, lineHeight: 1.45, background: 'rgba(2,6,23,.38)', border: '1px solid rgba(148,163,184,.18)', borderRadius: 8, padding: '7px 10px' }}><span style={{ fontSize: 9, fontWeight: 900, color: MUTED, textTransform: 'uppercase' }}>🏛 CIO reasoning</span><div style={{ marginTop: 3 }}>{(() => { const n = it.cio_narrative.replace(/^\[DUAL-CONSENSUS\]\s*/, ''); return n.length > 260 ? n.slice(0, 260) + '…' : n })()}</div></div>}
