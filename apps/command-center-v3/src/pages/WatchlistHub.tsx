        ...
        <div style={{ ...panel, marginBottom: 14 }}>
          ...
          {showDirectives && (() => {
            const renderDir = (d: any) => {
              ...
              return <div key={d.id} onClick={() => setFDir(fDir === String(d.id) ? 'all' : String(d.id))} title={fDir === String(d.id) ? 'click to clear this directive filter' : 'filter list to this directive'} style={{ padding: '7px 11px', background: 'var(--bg2)', borderRadius: 9, cursor: 'pointer', border: fDir === String(d.id) ? `1px solid ${PURPLE}` : '1px solid var(--border)' }}><div style={{ display: 'flex', gap: 6, alignItems: 'center' }}><Pill text={d.kind} color={PURPLE} /><span style={{ fontSize: 12, fontWeight: 800, color: TEXT0 }}>{dirName(d)}</span>{dirList(d) && <Pill text={`☰ ${dirList(d)}`} color="#22d3ee" tip="list membership" />}<Pill text={d.status} color={d.status === 'active' ? GREEN : d.status === 'paused' ? AMBER : MUTED} />{(d.aliases_n ?? 0) > 0 && <Pill text={`${d.aliases_n} alias`} color="#a78bfa" tip="near-dup labels absorbed by the family gate" />}</div><div style={{ fontSize: 10, color: MUTED, marginTop: 3 }}>{nHits} hits · {nStaged} staged · 7d {d.hits_7d ?? 0} · 30d {d.hits_30d ?? 0} · age {d.age_days ?? '—'}d · last {lastHit} <span style={{ fontFamily: 'monospace', color: PURPLE }} title="hits per week, last 8 weeks">{spark(d.spark_8w)}</span></div><div style={{ display: 'flex', gap: 5, marginTop: 4 }} onClick={e => e.stopPropagation()}>{d.status === 'active' && <button onClick={() => dirAction(d.id, 'pause')} style={{ fontSize: 9, padding: '2px 7px', borderRadius: 5, border: '1px solid var(--border)', background: 'transparent', color: MUTED, cursor: 'pointer' }}>Pause</button>}{(d.status === 'paused' || d.status === 'proposed') && <button onClick={() => dirAction(d.id, 'resume')} style={{ fontSize: 9, padding: '2px 7px', borderRadius: 5, border: '1px solid var(--border)', background: 'transparent', color: GREEN, cursor: 'pointer' }}>{d.status === 'proposed' ? 'Promote' : 'Resume'}</button>}{d.status !== 'archived' && <button onClick={() => dirAction(d.id, 'archive')} style={{ fontSize: 9, padding: '2px 7px', borderRadius: 5, border: '1px solid var(--border)', background: 'transparent', color: MUTED, cursor: 'pointer' }}>Archive</button>}{d.kind === 'trend' && d.status === 'active' && <button onClick={() => { const tid = window.prompt(`Merge #${d.id} '${dirName(d)}' into directive id:`); if (tid) dirAction(d.id, 'merge_into', Number(tid)) }} style={{ fontSize: 9, padding: '2px 7px', borderRadius: 5, border: '1px solid var(--border)', background: 'transparent', color: '#a78bfa', cursor: 'pointer' }}>Merge…</button>}</div></div>
            }
            return <>
              <div style={{ display: 'flex', gap: 8, marginBottom: 8, alignItems: 'center' }}>
                <button onClick={() => setDirSort(s => s === 'activity' ? 'needs_review' : 'activity')} style={{ fontSize: 10, fontWeight: 700, padding: '4px 10px', borderRadius: 6, border: '1px solid var(--border)', background: 'var(--bg2)', color: MUTED, cursor: 'pointer' }}>
                  Sort: {dirSort === 'activity' ? 'hits + staged' : 'needs review (coldest first)'}
                </button>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>{(dirSort === 'activity' ? actionableDirs : [...actionableDirs].sort((a: any, b: any) => String(a.last_hit_at || '0').localeCompare(String(b.last_hit_at || '0')))).map(renderDir)}</div>
