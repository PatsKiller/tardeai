                    ...
                    return (
                      <div style={{ borderTop: '1px solid var(--border)', paddingTop: 5, display: 'flex', flexDirection: 'column', gap: 3 }}>
                        ...
                        <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', alignItems: 'center' }}>
                          ...
                          {sc.analyst?.rating && <Pill text={`${String(sc.analyst.rating).replace('_', ' ')} · ${sc.analyst.opinions} analysts · target $${sc.analyst.target}${sc.analyst.upside_pct != null ? ` (${sc.analyst.upside_pct >= 0 ? '+' : ''}${sc.analyst.upside_pct}%)` : ''}`}
                            color={String(sc.analyst.rating).includes('buy') ? '#22c55e' : sc.analyst.rating === 'hold' ? '#f59e0b' : 'var(--text3)'}
                            tip={`Yahoo consensus ${sc.analyst.mean} · target range $${sc.analyst.target_low}–$${sc.analyst.target_high}${sc.analyst.distribution ? `\nvotes: ${sc.analyst.distribution.strong_buy ?? 0} strong buy · ${sc.analyst.distribution.buy ?? 0} buy · ${sc.analyst.distribution.hold ?? 0} hold · ${sc.analyst.distribution.sell ?? 0} sell` : ''}`} />}
                        </div>
