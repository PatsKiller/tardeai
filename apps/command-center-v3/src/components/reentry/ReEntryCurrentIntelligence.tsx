...
function deriveIntel(watch: any, card: any, mandate: ReEntryMandate, held: boolean): Intel {
  ...
  const asOf = textFrom(objects, ['last_enriched_at', 'computed_at', 'as_of', 'updated_at', 'quote_time', 'technicals.as_of', 'decision_packet_at'])
...
export default function ReEntryCurrentIntelligence() {
  ...
  const rows = useMemo(() => summaries.map(summary => {
    ...
    const intel = deriveIntel(watch, card, mandate, held.has(summary.symbol))
    ...
    return { ...summary, mandate, intel, move, alertsCount, resistance, analyst: analystMap[summary.symbol] ?? null, shared: sharedMap[summary.symbol] ?? null, classified }
