...
function deriveIntel(watch: any, card: any, held: boolean): Intel {
  ...
  const objects = [watch ?? {}, card ?? {}, packet ?? {}, mechanics ?? {}, packet?.technical_state ?? {}]
...
export default function ReEntryCurrentIntelligence() {
  ...
  const shown = rows.filter(row => {
    if (search.trim() && !`${row.symbol} ${row.intel.state} ${row.intel.action} ${row.classified} ${row.mandate.mandate} ${row.flags.join(' ')}`.toUpperCase().includes(search.trim().toUpperCase())) return false
  ...
  const counts = { symbols: rows.length, classified: rows.filter(row => row.classified === 'CLASSIFIED').length, ready: rows.filter(row => row.intel.state === 'READY TO REVIEW').length, near: rows.filter(row => row.intel.state === 'NEAR ENTRY').length, missing: rows.filter(row => row.completeness < 5).length }
  ...
  return <div style={{ ...panel, padding: 10 }}>
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}><div><div style={{ fontSize: 18, fontWeight: 900 }}>CURRENT RE-ENTRY INTELLIGENCE <HelpTip text="Real exits are reconciled across journal, redeploy book/history, stop-watch and the full-fidelity cache. Missing values remain explicit." /></div><div style={{ fontSize: 10.5, color: BB.text3 }}>Regime {regimeLabel} · {evidence.rows.length} reconciled exit events · {evidence.sources.filter(source => source.available).length}/{evidence.sources.length} sources reporting · advisory only</div></div><button onClick={refresh} style={{ ...button(false), marginLeft: 'auto' }}>{evidence.loading || evidence.refreshing ? 'REFRESHING…' : 'REFRESH ALL SOURCES'}</button></div>

    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,minmax(120px,1fr))', gap: 7, marginTop: 9 }}>{[
      ['EXITED SYMBOLS', counts.symbols, stateFilter === 'ALL' && classificationFilter === 'ALL', () => { setStateFilter('ALL'); setClassificationFilter('ALL') }],
      ...
      ['EVIDENCE GAPS', counts.missing, false, () => setSearch('')],
    ].map(([name, value, active, action]) => <button key={String(name)} onClick={action as () => void} style={{ ...panel, padding: 9, textAlign: 'left', cursor: 'pointer', background: active ? BB.blueDim : 'var(--bg2)', color: 'var(--text0)' }}><span style={{ color: BB.text3, fontSize: 9.5 }}>{name}</span><br /><b style={{ fontSize: 20 }}>{String(value)}</b></button>)}</div>

    <div style={{ display: 'grid', gridTemplateColumns: 'minmax(220px,1fr) 190px 170px auto auto auto', gap: 7, marginTop: 8 }}><input value={search} onChange={event => setSearch(event.target.value)} placeholder="Search symbol, state, mandate, flags…" style={field} /><select value={stateFilter} onChange={event => setStateFilter(event.target.value)} style={field}><option value="ALL">ALL CURRENT STATES</option>{['READY TO REVIEW', 'NEAR ENTRY', 'WAIT', 'CURRENTLY HELD', 'STALE', 'MISSING PLAN', 'MISSING MARKET'].map(state => <option key={state}>{state}</option>)}</select><select value={classificationFilter} onChange={event => setClassificationFilter(event.target.value)} style={field}><option value="ALL">ALL CLASSIFICATIONS</option><option>CLASSIFIED</option><option>AUTO-TAGGED</option><option>UNCLASSIFIED</option></select><button onClick={() => setSelected(Object.fromEntries(shown.map(row => [row.symbol, true])))} style={button(false)}>SELECT VISIBLE</button><button onClick={() => setSelected({})} style={button(false)}>CLEAR</button><button disabled={!selectedSymbols.length} onClick={() => classify(selectedSymbols)} style={{ ...button(Boolean(selectedSymbols.length)), opacity: selectedSymbols.length ? 1 : .5 }}>EDIT SELECTED {selectedSymbols.length}</button></div>

    <div style={{ overflowX: 'auto', marginTop: 8 }}><div style={{ minWidth: 1490 }}><div style={{ display: 'grid', gridTemplateColumns: '28px 180px 220px 125px 150px 170px 160px 150px 130px', gap: 8, padding: '7px 9px', borderBottom: '1px solid var(--border)', fontSize: 9.5, color: BB.text3, textTransform: 'uppercase' }}><span></span><span>Symbol / mandate</span><span>Current decision</span><span>Market</span><span>Exit evidence</span><span>Entry / resistance</span><span>Valuation / analyst</span><span>Evidence coverage</span><span>Actions</span></div>{shown.map(row => {
      ...
      return <div key={row.symbol}><div onClick={() => setExpanded(value => ({ ...value, [row.symbol]: !open }))} style={{ display: 'grid', gridTemplateColumns: '28px 180px 220px 125px 150px 170px 160px 150px 130px', gap: 8, padding: '9px', borderBottom: '1px solid var(--border)', alignItems: 'center', fontSize: 10.5, cursor: 'pointer', background: open ? 'var(--bg2)' : 'transparent' }}><input type="checkbox" checked={Boolean(selected[row.symbol])} onClick={event => event.stopPropagation()} onChange={event => setSelected(value => ({ ...value, [row.symbol]: event.target.checked }))} /><div><div><b style={{ fontSize: 14 }}>{row.symbol}</b> <span style={{ color: classTone, fontSize: 9.5 }}>{classificationLabel(row.classified)}</span> <span style={{ color: BB.text3 }}>{open ? '▾' : '▸'}</span></div><div style={{ marginTop: 3, color: mandateLabel === 'MANDATE NEEDED' ? BB.amber : BB.text2 }}>{mandateLabel}</div><div style={{ color: BB.text3 }}>{row.flags.join(' · ') || 'no strategy flags'}</div></div><div><span style={{ color: tone, fontWeight: 900 }}>{row.intel.state}</span><div style={{ marginTop: 3, fontWeight: 800 }}>{row.intel.action}</div><div style={{ color: BB.text3 }}>{row.intel.reason}</div></div><div><b>{money(row.intel.price)}</b><br /><span style={{ color: BB.text3 }}>RSI {row.intel.rsi === null ? '—' : row.intel.rsi.toFixed(1)} · {row.intel.trend}</span><br /><span style={{ color: BB.text3 }}>{age(row.intel.asOf)}</span></div><div><b>{row.rows.length} exits · {row.shares === null ? 'shares unavailable' : `${row.shares.toLocaleString()} sh`}</b><br /><span>avg {money(row.avgExit)} · {money(row.proceeds)}</span><br /><span style={{ color: BB.text3 }}>{row.latest.trade_date ?? 'date unavailable'}</span></div><div><b>{row.intel.entryLow === null ? 'entry unavailable' : row.intel.entryLow === row.intel.entryHigh ? money(row.intel.entryLow) : `${money(row.intel.entryLow)}–${money(row.intel.entryHigh)}`}</b><br /><span style={{ color: BB.text3 }}>stop {money(row.intel.stop)} · target {money(row.intel.target)}</span><br /><span>{row.resistance.state} {money(row.resistance.level)} · {row.resistance.distancePct === null ? 'distance —' : `${row.resistance.distancePct >= 0 ? '+' : ''}${row.resistance.distancePct.toFixed(1)}%`}</span></div><div><b>P/E {pe === null ? '—' : pe.toFixed(2)} · Fwd {fpe === null ? '—' : fpe.toFixed(2)}</b><br /><span>{rec}</span><br /><span style={{ color: BB.text3 }}>{row.analyst?.n ?? '—'} analysts · target {row.analyst?.target == null ? '—' : money(Number(row.analyst.target))}</span></div><div><b>{row.completeness}/8 fields present</b><div style={{ marginTop: 4, height: 4, background: 'var(--border)', borderRadius: 2 }}><div style={{ width: `${row.completeness / 8 * 100}%`, height: '100%', background: row.completeness >= 7 ? BB.green : row.completeness >= 5 ? BB.blue : BB.amber, borderRadius: 2 }} /></div><div style={{ color: BB.text3, marginTop: 4 }}>{row.latest.import_source || 'source unavailable'}</div></div><div onClick={event => event.stopPropagation()}><button onClick={() => classify([row.symbol])} style={button(true)}>EDIT CLASSIFICATION</button><button onClick={() => { window.location.href = `/v3/rotation?symbol=${encodeURIComponent(row.symbol)}` }} style={{ ...button(false), marginTop: 5 }}>ROTATION</button><div style={{ color: BB.text3, marginTop: 5 }}>alerts {row.alertCount}</div></div></div>
      {open && <div style={{ padding: '10px 14px 14px 42px', background: 'var(--bg2)', borderBottom: '1px solid var(--border)', display: 'grid', gridTemplateColumns: '1.15fr .85fr', gap: 18 }}><div><div style={{ fontSize: 9.5, fontWeight: 900, color: BB.text3, marginBottom: 6 }}>RECONCILED EXIT HISTORY — TRAILING 12 MONTHS</div>{row.rows.map(exit => { const event = normalizedEvent(exit, events[exit.event_key]); return <div key={exit.event_key} style={{ display: 'grid', gridTemplateColumns: '85px 120px 90px 90px 100px 1fr', gap: 7, fontSize: 10, padding: '5px 0', borderBottom: '1px solid var(--border)' }}><span>{exit.trade_date ?? '—'}</span><span>{exit.account ?? '—'}</span><span>{rowShares(exit) === null ? '—' : `${rowShares(exit)?.toLocaleString()} sh`}</span><span>{money(rowPrice(exit))}</span><span>{money(finite(exit.proceeds_usd))}</span><span><b>{event.eventType.replace(/_/g, ' ').toUpperCase()}</b><br /><span style={{ color: BB.text3 }}>{event.reason || 'reason unavailable'} · {exit.import_source || 'source unavailable'}</span></span></div>})}</div><div><div style={{ fontSize: 9.5, fontWeight: 900, color: BB.text3, marginBottom: 6 }}>CURRENT WATCH / PORTFOLIO CONTEXT</div><div style={{ fontSize: 10.5, lineHeight: 1.55 }}><b>Mandate:</b> {mandateLabel}<br /><b>Priority:</b> {row.mandate.priority}<br /><b>Thesis:</b> {row.mandate.thesis || 'No operator thesis saved.'}<br /><b>Watch recommendation:</b> {text(row.watch?.synthesis_recommendation, row.watch?.latest_recommendation, 'unavailable').replace(/_/g, ' ')}<br /><b>Sector:</b> {text(row.watch?.profile_sector, row.card?.sector, 'unavailable')}<br /><b>Catalyst:</b> {text(row.watch?.catalyst_headline, 'unavailable')}<br /><b>Earnings:</b> {text(row.watch?.earnings_date, row.watch?.next_earnings_date, 'unavailable')}<br /><b>Resistance source:</b> {row.resistance.source} — {row.resistance.reason}<br /><b>Disposition:</b> {normalizedEvent(row.latest, events[row.latest.event_key]).eventType.replace(/_/g, ' ')}</div></div></div>}
      </div>
    })}</div></div>
