...
function parseViewParam(raw: string | null): ViewMode | null {
  ...
  if (v === 'all' || v === 'material' || v === 'classified' || v === 'ready') return v
  return null
}
...
function loadSessionView(): ViewMode {
  try {
    ...
    // Accept prior uppercase session values from adjacent branches
    if (raw === 'ALL') return 'all'
    if (raw === 'MATERIAL') return 'material'
    if (raw === 'CLASSIFIED') return 'classified'
    if (raw === 'READY_NEAR' || raw === 'READY') return 'ready'
  } catch { /* private mode */ }
  return 'material'
}
...
export default function ReEntryCurrentIntelligence() {
  ...
  /** Preserve unrelated params (e.g. classify overlay) when writing view/state/symbol. */
  ...
  // Read URL on mount / back-forward; write path is via setters above.
  useEffect(() => {
    ...
    if (sym) {
      setSearch(sym)
      setExpanded(prev => (prev[sym] ? prev : { ...prev, [sym]: true }))
    }
  ...
  const shown = rows.filter(row => {
    if (viewMode === 'material' && Math.abs(Number(row.proceeds) || 0) < MATERIAL_PROCEEDS_MIN) return false
  ...
  const materialCount = rows.filter(row => Math.abs(Number(row.proceeds) || 0) >= MATERIAL_PROCEEDS_MIN).length
  const counts = {
    symbols: rows.length,
  ...
  const refreshMarket = () => { cards.refetch(); holdings.refetch(); setWatchReload(n => n + 1) }
  ...
  const viewChips: { id: ViewMode; label: string; count: number }[] = [
    ...
    { id: 'ready', label: 'Ready+Near', count: counts.ready + counts.near },
  ...
  return <div style={{ ...panel, padding: 10 }}>
    ...
    <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 9, alignItems: 'center' }}>
      ...
      {viewChips.map(chip => (
        <button
          ...
          title={chip.id === 'material' ? `Material: trailing exit proceeds ≥ $${MATERIAL_PROCEEDS_MIN.toLocaleString()}` : undefined}
        >
    ...
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,minmax(120px,1fr))', gap: 7, marginTop: 9 }}>{[
      ...
      <button
        key={String(name)}
        type="button"
        title={String(name) === 'INCOMPLETE EVIDENCE' ? 'event gaps or <8/8 fields' : undefined}
        onClick={action as () => void}
        style={{ ...panel, padding: 9, textAlign: 'left', cursor: 'pointer', background: active ? BB.blueDim : 'var(--bg2)', color: 'var(--text0)' }}
      >
        <span style={{ color: BB.text3, fontSize: 10 }}>{String(name)}</span>
        {String(name) === 'INCOMPLETE EVIDENCE' && <div style={{ color: BB.text3, fontSize: 10, marginTop: 2, fontWeight: 500 }}>event gaps or &lt;8/8 fields</div>}
    ...
    <div style={{ overflowX: 'auto', marginTop: 8 }}><div style={{ minWidth: 1530 }}><div style={{ display: 'grid', gridTemplateColumns: '28px 180px 220px 125px 170px 170px 160px 160px 145px', gap: 8, padding: '7px 9px', borderBottom: '1px solid var(--border)', fontSize: 10, color: BB.text3, textTransform: 'uppercase' }}><span></span><span>Symbol / mandate</span><span>Current decision</span><span>Market</span><span>Exit evidence</span><span>Entry / resistance</span><span>Valuation / analyst</span><span>Evidence audit</span><span>Actions</span></div>{shown.map(row => {
      ...
      return <div key={row.symbol} id={`reentry-intel-${row.symbol}`}>
        <div role="button" tabIndex={0} aria-expanded={open} onKeyDown={event => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); setExpanded(value => ({ ...value, [row.symbol]: !open })) } }} onClick={() => setExpanded(value => ({ ...value, [row.symbol]: !open }))} style={{ display: 'grid', gridTemplateColumns: '28px 180px 220px 125px 170px 170px 160px 160px 145px', gap: 8, padding: '9px', borderBottom: '1px solid var(--border)', alignItems: 'center', fontSize: 10.5, cursor: 'pointer', background: open ? 'var(--bg2)' : 'transparent', boxShadow: open ? `inset 3px 0 0 ${BB.blue}` : undefined }}>
          ...
          <div onClick={event => event.stopPropagation()}>
            ...
            {missingMarket && (
              <button
                ...
                title="Re-fetch watchlist, holdings, and symbol cards. Does not invent prices."
    ...
    <div style={{ marginTop: 8, fontSize: 10, color: BB.text3 }}>
      Material threshold: <b>$5,000</b> trailing exit proceeds (view default stored in session as {VIEW_STORAGE_KEY}).
      {' '}Shareable URL: <code>?view=</code> · <code>?state=</code> · <code>?symbol=</code> (replace).
    </div>
  </div>
}
