...
export default function ReEntryCurrentIntelligence() {
  ...
  return <div style={{ ...panel, padding: 10 }}>
    ...
    <div style={{ ...panel, marginTop: 8, padding: 8, background: 'var(--bg2)', fontSize: 10.5 }}><b>Source audit:</b> {evidence.sources.map(source => `${source.label} ${source.rows}`).join(' · ')}. A missing value means no compatible event-level or ticker-aggregate fact was found; derived shares/price/proceeds are labeled in the expanded audit.</div>
