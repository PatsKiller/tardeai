...
export default function ReEntryCurrentIntelligence() {
  ...
  const rows = useMemo(() => summaries.map(summary => {
    ...
    // Suppressed = every exit is operator-suppressed. Broker history is never deleted;
    // the symbol just leaves the default re-entry queue.
    ...
    return { ...summary, mandate, watch, card, intel, resistance, classified, flags, completeness, alertCount, suppressed, analyst: analystMap[summary.symbol] ?? null }
  ...
  return <div style={{ ...panel, padding: 10 }}>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: 'minmax(200px,1fr) 180px 165px 205px auto auto auto', gap: 7, marginTop: 8 }}><input value={search} onChange={event => setSearch(event.target.value)} placeholder="Search symbol, state, source or missing field…" style={field} /><select value={stateFilter} onChange={event => setStateFilter(event.target.value)} style={field}><option value="ALL">ALL CURRENT STATES</option>{['READY TO REVIEW', 'NEAR ENTRY', 'WAIT', 'CURRENTLY HELD', 'STALE', 'MISSING PLAN', 'MISSING MARKET'].map(state => <option key={state}>{state}</option>)}</select><select value={classificationFilter} onChange={event => setClassificationFilter(event.target.value)} style={field}><option value="ALL">ALL CLASSIFICATIONS</option><option>CLASSIFIED</option><option>AUTO-TAGGED</option><option>UNCLASSIFIED</option></select><select value={queueFilter} onChange={event => setQueueFilter(event.target.value as typeof queueFilter)} title="Suppressed symbols are ones you chose not to track. Set it per symbol in CLASSIFY → QUEUE DISPOSITION → SUPPRESS. Broker history is never deleted." style={field}><option value="ACTIVE">ACTIVE · HIDE SUPPRESSED</option><option value="SUPPRESSED">SUPPRESSED ONLY</option><option value="ALL">ALL · INCLUDE SUPPRESSED</option></select><button onClick={() => setSelected(Object.fromEntries(shown.map(row => [row.symbol, true])))} style={button(false)}>SELECT VISIBLE</button><button onClick={() => setSelected({})} style={button(false)}>CLEAR</button><button disabled={!selectedSymbols.length} onClick={() => classify(selectedSymbols)} style={{ ...button(Boolean(selectedSymbols.length)), opacity: selectedSymbols.length ? 1 : .5 }}>EDIT SELECTED {selectedSymbols.length}</button></div>
