...
const VIEW_KEY = 'cc-reentry-view-v1'
const MATERIAL_PROCEEDS = 5000
type ViewMode = 'all' | 'material' | 'classified' | 'ready'

function readView(): ViewMode {
  try {
    const raw = sessionStorage.getItem(VIEW_KEY)
    if (raw === 'all' || raw === 'material' || raw === 'classified' || raw === 'ready') return raw
  } catch { /* ignore */ }
  return 'material'
}
...
function deriveIntel(watch: any, card: any, held: boolean, holding?: any): Intel {
  ...
  const objects = [watch ?? {}, card ?? {}, holding ?? {}, packet ?? {}, mechanics ?? {}, packet?.technical_state ?? {}, packet?.current_input_snapshot ?? {}]
  const price = numberFrom(objects, ['price', 'price_live', 'latest_price', 'price_enriched', 'last_price', 'current_price', 'quote.last', 'quote.price'])
  const asOf = textFrom(objects, ['price_as_of', 'last_enriched_at', 'last_validated_at', 'computed_at', 'decision_packet_at', 'as_of'])
  const rsi = numberFrom(objects, ['rsi', 'rsi_14', 'technical.rsi', 'technicals.rsi', 'current_rsi'])
  const trend = textFrom(objects, ['trend_state', 'trend_direction', 'overall_direction', 'technical_state.overall_direction', 'technicals.trend', 'trend']).replace(/_/g, ' ').toUpperCase() || 'UNAVAILABLE'
  ...
  const entry = numberFrom(objects, ['entry_limit', 'reentry_price', 'entry_price', 'ideal_entry', 'mechanics.entry', 'selected_family.mechanics.entry'])
  const support = numberFrom(objects, ['support', 'support_level'])
  if (entryLow === null) entryLow = entry ?? support
  if (entryHigh === null) entryHigh = entry ?? support
  if (entryLow !== null && entryHigh !== null && entryLow > entryHigh) [entryLow, entryHigh] = [entryHigh, entryLow]
  const stop = numberFrom(objects, ['entry_stop', 'reentry_stop', 'stop_price', 'stop_loss', 'mechanics.stop', 'selected_family.mechanics.stop'])
  const target = numberFrom(objects, ['entry_target', 'reentry_target', 'target_price', 'resistance', 'mechanics.target', 'selected_family.mechanics.target'])
...
export default function ReEntryCurrentIntelligence() {
  ...
  useEffect(() => {
    try { sessionStorage.setItem(VIEW_KEY, viewMode) } catch { /* ignore */ }
  }, [viewMode])

  useEffect(() => {
    const next = new URLSearchParams(params)
    if (viewMode === 'material') next.delete('view'); else next.set('view', viewMode)
    if (stateFilter === 'ALL') next.delete('state'); else next.set('state', stateFilter)
    const sym = search.trim().toUpperCase()
    if (sym && /^[A-Z.]{1,12}$/.test(sym) && Object.keys(expanded).some(k => expanded[k] && k === sym)) next.set('symbol', sym)
    else if (!sym) next.delete('symbol')
    const same = next.toString() === params.toString()
    if (!same) setParams(next, { replace: true })
  }, [viewMode, stateFilter, search, expanded])
  ...
  const holdingList: any[] = unwrap(holdings.data)?.holdings ?? []
  const heldSet = new Set<string>(holdingList.filter((row: any) => Number(row.shares ?? row.quantity ?? 0) > 0).map((row: any) => String(row.symbol || '').toUpperCase()))
  const holdingBySymbol = useMemo(() => {
    const map: Record<string, any> = {}
    for (const row of holdingList) {
      const sym = String(row.symbol || '').toUpperCase()
      if (sym && !map[sym]) map[sym] = row
    }
    return map
  }, [holdings.data])
  ...
  }), [summaries, mandatesPref.data, eventsPref.data, dispositionsPref.data, watchMap, cards.data, holdings.data, resistancePref.data, analyst.data, alerts.data, holdingBySymbol])

  const shown = rows.filter(row => {
    if (viewMode === 'material' && !(row.proceeds >= MATERIAL_PROCEEDS)) return false
    if (viewMode === 'classified' && row.classified !== 'CLASSIFIED') return false
    if (viewMode === 'ready' && row.intel.state !== 'READY TO REVIEW' && row.intel.state !== 'NEAR ENTRY') return false
  ...
  const viewChips: { id: ViewMode; label: string }[] = [
    { id: 'material', label: 'Material' },
    { id: 'classified', label: 'Classified' },
    { id: 'ready', label: 'Ready+Near' },
    { id: 'all', label: 'All' },
  ]
  ...
  return <div style={{ ...panel, padding: 10 }}>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,minmax(120px,1fr))', gap: 7, marginTop: 9 }}>{[
      ...
      ['CLASSIFIED', counts.classified, classificationFilter === 'CLASSIFIED' || viewMode === 'classified', () => { setViewMode('classified'); setClassificationFilter('ALL') }],
    ...
    ].map(([name, value, active, action]) => <button key={String(name)} title={name === 'INCOMPLETE EVIDENCE' ? 'event gaps or <8/8 current fields' : undefined} onClick={action as () => void} style={{ ...panel, padding: 9, textAlign: 'left', cursor: 'pointer', background: active ? BB.blueDim : 'var(--bg2)', color: 'var(--text0)' }}><span style={{ color: BB.text3, fontSize: 10 }}>{String(name)}</span><br /><b style={{ fontSize: 20 }}>{String(value)}</b>{name === 'INCOMPLETE EVIDENCE' && <div style={{ fontSize: 9, color: BB.text3, marginTop: 2 }}>event gaps or <8/8 fields</div>}</button>)}</div>

    <div style={{ display: 'flex', gap: 6, marginTop: 8, flexWrap: 'wrap', alignItems: 'center' }}>
      <span style={{ fontSize: 10, color: BB.text3, fontWeight: 800 }}>VIEW</span>
      {viewChips.map(chip => <button key={chip.id} onClick={() => setViewMode(chip.id)} style={button(viewMode === chip.id)}>{chip.label}</button>)}
      <span style={{ fontSize: 10, color: BB.text3, marginLeft: 4 }}>showing {shown.length} of {rows.length}</span>
    </div>
    ...
    <div style={{ marginTop: 8, fontSize: 10, color: BB.text3 }}>Material view threshold: trailing exit proceeds ≥ ${MATERIAL_PROCEEDS.toLocaleString()}. Default view is Material (session). Deep link: ?view=material|classified|ready|all&state=…&symbol=TSLA</div>
  </div>
}
