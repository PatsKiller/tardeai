...
import { NewsArticleCurated, ResearchGapCurated, LibraryItemCurated, isCuratedIntelligenceEndpoint } from './IntelligenceDrawerCurated'
...
export default function DetailDrawer({ ctx, onClose }: Props) {
  ...
  useEffect(() => { setShowRaw(false) }, [ctx?.endpoint, ctx?.title])
  useEffect(() => {
    setIntel(null)
    if (!ctx) return
    let cancelled = false
  ...
  const newsMode = (ctx?.endpoint || '').includes('/news/articles')
  const researchMode = (ctx?.endpoint || '').includes('/research-topics')
  const libraryMode = (ctx?.endpoint || '').includes('/intelligence/library')
  const curatedMode = isCuratedIntelligenceEndpoint(ctx?.endpoint)
  const showStockIntel = !chainMode && !journeyMode && !curatedMode && !!drawerSymbol
  ...
  return <div style={{ position: 'fixed', inset: 0, zIndex: 1000, display: 'flex', justifyContent: 'flex-end', background: 'rgba(2,6,23,.22)' }} onClick={onClose}>
    <div className={terminalUi ? 'cc-drawer' : undefined} onClick={e => e.stopPropagation()} style={{ width: 'min(1320px, 96vw)', height: '100vh', background: terminalUi ? '#0a0e1a' : 'linear-gradient(180deg, #111827, #0f172a)', borderLeft: `1px solid ${terminalUi ? '#1e293b' : 'rgba(148,163,184,.22)'}`, display: 'flex', flexDirection: 'column', boxShadow: '-10px 0 40px rgba(0,0,0,.55)' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: 18, display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        {newsMode && primary.title && (
          <Section title="News article" subtitle="Curated view — actions below" accent={BLUE}>
            <NewsArticleCurated row={primary} />
          </Section>
        )}
        {researchMode && !newsMode && (
          <Section title="Research topic" subtitle="Gap or auto-research brief" accent={AMBER}>
            <ResearchGapCurated row={primary} />
          </Section>
        )}
        {libraryMode && (
          <Section title="Library item" subtitle="Unified intelligence corpus" accent={TEAL}>
            <LibraryItemCurated row={primary} />
          </Section>
        )}
