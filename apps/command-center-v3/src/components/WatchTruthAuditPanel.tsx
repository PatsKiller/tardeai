...
export default function WatchTruthAuditPanel() {
  ...
  return <div style={{ ...panel, padding: 10, margin: '8px 0 12px', borderColor: T.link }}>
    ...
    {selectedItem && selectedVal && selectedTicket && <div style={{ ...panel, marginTop: 9, padding: 10, borderColor: stateColor(selectedTicket.reconciled) }}><div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}><b style={{ fontSize: 14 }}>{selected} — EVIDENCE & INDEPENDENT REVIEW</b><span style={{ color: selectedItem.starred ? BB.green : BB.amber, fontSize: 10 }}>{originLabel(selectedItem)}</span><span style={{ marginLeft: 'auto', color: BB.text3, fontSize: 10 }}>P/E source: {selectedVal.available ? 'Finviz enrichment / blind facts' : selectedVal.notApplicable ? 'legitimately not applicable' : 'unavailable'} · {age(selectedVal.asOf)}</span></div>
      ...
      {premium && <div style={{ ...panel, marginTop: 8, padding: 9, borderColor: premium.available ? T.extIntel.hermes : BB.amber, background: 'var(--bg2)' }}><div style={{ fontSize: 11, fontWeight: 900, color: premium.available ? T.extIntel.hermes : BB.amber }}>PAID EXPERT COST & CAPABILITY PREVIEW</div>{premium.available ? <><div style={{ fontSize: 10, marginTop: 5 }}>{premium.provider}/{premium.model} · estimated ${Number(premium.est_cost_usd).toFixed(4)} · input {premium.est_input_tokens} · output {premium.est_output_tokens} · expected latency {premium.expected_latency_s}s · daily budget ${premium.daily_budget_usd} · monthly budget ${premium.monthly_budget_usd}</div><div style={{ fontSize: 10, color: BB.text3, marginTop: 4 }}>{premium.scope}</div><label style={{ display: 'block', fontSize: 10, color: BB.text3, marginTop: 7 }}>TYPE EXACTLY: <code>{premium.confirm_with}</code><input value={confirmation} onChange={event => setConfirmation(event.target.value)} style={{ width: '100%', boxSizing: 'border-box', marginTop: 4, fontSize: 11, padding: '6px 8px', background: 'var(--bg1)', border: '1px solid var(--border)', color: BB.text0 }} /></label><button disabled={confirmation !== premium.confirm_with || Boolean(reviewBusy)} onClick={() => void runPremium()} style={{ ...button(true), marginTop: 7, color: T.extIntel.hermes, borderColor: T.extIntel.hermes, opacity: confirmation === premium.confirm_with ? 1 : .5 }}>CONFIRM PAID REVIEW</button></> : <div style={{ fontSize: 10, marginTop: 5, color: BB.amber }}>{premium.reason}<br />Registry entries: {(premium.providers_listed ?? []).join(', ') || 'none'}. No paid call was made.</div>}</div>}
    </div>}
  </div>
}
