      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(650px,1.25fr) minmax(350px,.75fr)', gap: 10, marginTop: 9, alignItems: 'start' }}>
        ...
        <aside style={{ ...panel, padding: 10, position: 'sticky', top: 8 }} aria-live="polite">
          {!selectedRow ? <div style={{ color: BB.text3 }}>Choose a symbol from the queue to open its evidence and actions.</div> : <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 6, marginTop: 9 }}><button disabled={Boolean(reviewBusy)} onClick={() => void runReview('local', 'Local critic')} style={button(false)}>{reviewBusy === 'Local critic' ? 'LOCAL…' : 'RUN LOCAL'}</button><button disabled={Boolean(reviewBusy)} onClick={() => void runReview('grok', 'Grok OAuth')} style={button(false)}>{reviewBusy === 'Grok OAuth' ? 'GROK…' : 'RUN GROK OAUTH'}</button><button disabled={Boolean(reviewBusy)} onClick={() => void runReview('chatgpt', 'ChatGPT OAuth')} style={button(false)}>{reviewBusy === 'ChatGPT OAuth' ? 'CHATGPT…' : 'RUN CHATGPT OAUTH'}</button><button disabled={Boolean(reviewBusy)} onClick={() => void runReview('deepseek-flash', 'DeepSeek Flash')} style={button(false)}>{reviewBusy === 'DeepSeek Flash' ? 'DEEPSEEK…' : 'DEEPSEEK FLASH'}</button><button disabled={Boolean(reviewBusy)} onClick={() => void runReview('deepseek-v4-pro', 'DeepSeek V4 Pro')} style={{ ...button(false), borderColor: '#a855f766', color: '#a855f7', background: '#a855f714' }}>{reviewBusy === 'DeepSeek V4 Pro' ? 'DEEPSEEK V4…' : 'DEEPSEEK V4 PRO'}</button><button disabled={Boolean(reviewBusy)} onClick={() => void runReview('local,grok,chatgpt,deepseek-flash', 'All critics')} style={button(true)}>{reviewBusy === 'All critics' ? 'ALL…' : 'RUN ALL'}</button></div>
