...
export default function DecisionPacketBand({ packet, generatedAt, actionPolicy, held, onRefresh, onRefreshStrategy, refreshing, onPrimary, onAlert }: Props) {
  ...
  return (
    <section aria-label="Decision strategy desk" data-watch-decision-contract="watch-quality-governance-v1" onClick={event => event.stopPropagation()} style={{ borderLeft: `3px solid ${accent}`, borderBottom: `1px solid ${BB.border}`, background: BB.bg }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', padding: '12px 16px', background: 'linear-gradient(180deg, rgba(0,0,0,.25), rgba(0,0,0,.45))', borderTop: `1px solid ${BB.border}` }}>
        <button onClick={event => { event.stopPropagation(); if (pres.state === 'REFRESH') { if (onRefreshStrategy) onRefreshStrategy(event); else if (onRefresh) onRefresh(event) } else if (pres.state === 'WAIT' && onAlert) onAlert(pres, event); else if (onPrimary) onPrimary(pres, event) }} disabled={refreshing && pres.state === 'REFRESH'} style={{ ...terminalButton(pres.state === 'READY' || pres.state === 'REFRESH' ? (pres.state === 'READY' ? 'success' : 'primary') : 'secondary'), flex: '1 1 160px', minWidth: 150, textAlign: 'center', ...(refreshing && pres.state === 'REFRESH' ? { opacity: .6, cursor: 'wait' } : {}) }}>{refreshing && pres.state === 'REFRESH' ? 'Refreshing strategy…' : pres.primaryCta}</button>
        {!pres.stale && onRefreshStrategy && <button onClick={event => { event.stopPropagation(); onRefreshStrategy(event) }} disabled={refreshing} style={terminalButton('secondary')}>{refreshing ? 'Refreshing…' : 'Refresh Strategy'}</button>}
        {onRefresh && <button onClick={event => { event.stopPropagation(); onRefresh(event) }} style={terminalButton('ghost')}>Refresh Inputs</button>}
        {pres.stale && pres.mechanics?.previous && <button onClick={event => { event.stopPropagation(); setPrevOpen(value => !value) }} style={terminalButton('ghost')}>{prevOpen ? 'Hide Previous Strategy' : 'View Previous Strategy'}</button>}
        {pres.whyNot.length > 0 && <button onClick={event => { event.stopPropagation(); setWhyOpen(value => !value) }} style={terminalButton('ghost')}>{whyOpen ? 'Hide alternatives' : 'Why not alternatives?'}</button>}
        <button onClick={event => { event.stopPropagation(); setDetails(value => !value) }} style={terminalButton('ghost')}>{details ? 'Hide audit' : 'Details & audit'}</button>
