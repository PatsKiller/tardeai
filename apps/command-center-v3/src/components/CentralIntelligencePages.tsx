...
export default function CentralIntelligencePages({ mode, onDrill }: Props) {
  ...
  const items = useMemo<IntelItem[]>(() => {
    ...
    ;(cmd.triggered_detail ?? []).slice(0, 20).forEach((s: any) => { const cp = Number(s.current_price), sp = Number(s.stop_price ?? s.stop); const ok = Number.isFinite(cp) && Number.isFinite(sp) && sp > 0; const conf = ok ? Math.min(.95, .64 + Math.min(Math.abs(cp - sp) / sp * 3, .31)) : ((s.stop_price ?? s.stop) && s.symbol ? .66 : .48); add({ source: '/api/v2/command', type: 'telegram/action', symbol: s.symbol, title: `${s.symbol} triggered stop from command feed`, summary: `Stop ${s.stop_price ?? s.stop ?? '—'} · ${s.account ?? ''}${ok ? ` · current ${cp}` : ''}`, severity: 'critical', confidence: conf, raw: s, action: 'confirm broker state' }) })
