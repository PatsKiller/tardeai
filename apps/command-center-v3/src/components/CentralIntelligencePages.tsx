...
function qualityReasons(it: IntelItem) {
  ...
  if (/risk|telegram|open-trade/.test(it.type)) out.push('requires broker/account readback')
...
export default function CentralIntelligencePages({ mode, onDrill }: Props) {
  ...
  const askLm = async () => {
    ...
    try { const r = await fetch('/api/v2/agents/intelligence-feedback', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) }); if (!r.ok) throw new Error(String(r.status)); const j = await r.json(); setLmStatus(`Submitted: ${j.status ?? 'ok'}`) }
    catch (e: any) { const local = JSON.parse(localStorage.getItem('tradeai.intelFeedback.v1') || '[]'); local.unshift({ at: new Date().toISOString(), payload }); localStorage.setItem('tradeai.intelFeedback.v1', JSON.stringify(local.slice(0, 50))); setLmStatus(`No backend feedback endpoint yet (${e.message}); saved locally for agent handoff.`) }
  }
