...
test('defense desktop and narrow render gate', async ({ page }) => {
  ...
  console.log(`DEFENSE_URL=${page.url()}`)
  console.log(`DEFENSE_MAIN=${(await page.locator('main').innerText()).slice(0, 6000)}`)
  await page.screenshot({ path: 'render-artifacts/defense-route-debug.png', fullPage: true })
  await expect(page.locator('main').getByText('Defense Desk', { exact: true })).toBeVisible()
  await expect(page.locator('main').getByText('Institutional rotation brief', { exact: true })).toBeVisible()
  await expect(page.locator('main').locator('b').filter({ hasText: 'No governed add card is active.' }).first()).toBeVisible()
  await expect(page.locator('main').getByText(/WITHHELD/).first()).toBeVisible()
...
test('sectors desktop and narrow render gate', async ({ page }) => {
  ...
  await expect(page.locator('main').getByText('Sectors & Industries', { exact: true })).toBeVisible()
  await expect(page.locator('main').getByText(/screen matches/i).first()).toBeVisible()
