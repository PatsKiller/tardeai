...
test('defense desktop and narrow render gate', async ({ page }) => {
  ...
  await expect(page.getByText('Institutional rotation brief')).toBeVisible()
  await expect(page.getByText(/No governed add card is active/)).toBeVisible()
...
test('sectors desktop and narrow render gate', async ({ page }) => {
  ...
  await expect(page.getByText('Sectors & Industries')).toBeVisible()
  await expect(page.getByText(/screen matches/i).first()).toBeVisible()
  await expect(page.getByText(/THIN COVERAGE/).first()).toBeVisible()
  await assertNoHorizontalOverflow(page)
