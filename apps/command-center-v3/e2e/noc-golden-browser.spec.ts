...
test.describe('NOC golden browser acceptance', () => {
  ...
  test('operator surfaces render NOC, THIN, research ops and receipts', async ({ page }) => {
    ...
    await expect(card).not.toContainText('vsymbol_noc@v2')
    await expect(card).not.toContainText('none_on_record')
    await expect(card).not.toContainText('THESIS_STRENGTHENED')
    await expect(card.getByText('Supporting evidence', { exact: true })).toBeVisible()
