...
export default function Portfolio() {
  ...
  const positions = useMemo(() => {
    const raw = hd?.holdings ?? []
    return acctFilter === 'all' ? raw : raw.filter(p => p.account === acctFilter)
  }, [hd, acctFilter])
  ...
  const columns = [
    ...
    { key: 'day_change_pct', label: 'Day %', width: 50, align: 'right' as const, sortKey: (r: Holding) => r.day_change_pct, render: (r: Holding) => (
      <span style={{ color: deltaColor(r.day_change_pct) }}>{fmtPct(r.day_change_pct)}</span>
    )},
  ...
  return (
    <>
      ...
      <div style={{ display: 'flex', gap: 3, marginBottom: 8 }}>
        {accounts.map(a => (
          <button key={a} onClick={() => setAcctFilter(a)} style={{
            padding: '3px 9px', fontSize: 10, border: '1px solid var(--border)', borderRadius: 'var(--radius)',
            background: acctFilter === a ? 'var(--accent-dim)' : 'var(--bg-card)',
      ...
      <DataGrid columns={columns} data={positions} rowKey={r => r.symbol + ':' + r.account}
        onRowClick={r => setSelected(r.symbol + ':' + r.account === selected ? null : r.symbol + ':' + r.account)}
          ...
          <>
            <DrawerSection title="Position">
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 4 }}>
                ...
                <DrawerStat label="Allocation" value={sel.portfolio_pct.toFixed(1) + '%'} color={sel.portfolio_pct >= 12 ? 'var(--red)' : undefined} />
                <DrawerStat label="Day Change" value={`${sel.day_change >= 0 ? '+' : ''}${fmt$(sel.day_change)} (${fmtPct(sel.day_change_pct)})`} color={deltaColor(sel.day_change_pct)} />
