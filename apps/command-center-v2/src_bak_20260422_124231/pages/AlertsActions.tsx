...
interface BehavData { has_data: boolean; best_day: string; worst_day: string; [key: string]: unknown }
interface StressData { has_data: boolean; scenarios: { name: string; impact_pct: number; impact_dollar: number }[]; worst_case_scenario: string; worst_case_loss: number }
    ...
    <>
        ...
        <>
          ...
          <Card compact>
            <div style={{ fontSize: 10, color: 'var(--text2)' }}>
              Best day: <strong style={{ color: 'var(--green)' }}>{behav.best_day}</strong> | Worst day: <strong style={{ color: 'var(--red)' }}>{behav.worst_day}</strong>
            </div>
          </Card>
        </>
      )}
      ...
      {stress?.has_data && stress.scenarios && (
        <>
          ...
          <Card compact>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              {stress.scenarios.slice(0, 4).map((s, i) => (
                <div key={i} style={{ display: 'flex', gap: 8, padding: '3px 0', borderBottom: '1px solid var(--border-subtle)', fontSize: 10 }}>
                  <span style={{ flex: 1, color: 'var(--text2)' }}>{s.name}</span>
                  <span style={{ color: 'var(--red)', fontWeight: 600 }}>{fmtPct(s.impact_pct)}</span>
                  <span style={{ color: 'var(--red)', width: 70, textAlign: 'right' }}>{fmt$(s.impact_dollar)}</span>
                </div>
              ))}
            </div>
            {stress.worst_case_scenario && (
              <div style={{ marginTop: 6, fontSize: 9, color: 'var(--text3)' }}>
                Worst case: {stress.worst_case_scenario} ({fmt$(stress.worst_case_loss)})
              </div>
            )}
          </Card>
        </>
